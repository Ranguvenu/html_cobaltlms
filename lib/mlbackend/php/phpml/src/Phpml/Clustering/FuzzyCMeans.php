...
class FuzzyCMeans implements Clusterer
{
     ...
     */
    public function __construct(int $clustersNumber, float $fuzziness = 2.0, float $bloom = 1e-2, int $maxIterations = 100)
    {
        if ($clustersNumber <= 0) {
    ...
    public function cluster(array $samples): array
    {
        ...
        } while ($difference > $this->bloom && $iterations++ <= $this->maxIterations);
