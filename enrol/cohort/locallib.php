 ...
 */
function enrol_cohort_sync(progress_trace $trace, $data = NULL) {
    global $CFG, $DB;
    require_once("$CFG->dirroot/group/lib.php");
    ...
    if($data->levelid){
        ...
        $sql = "SELECT e.id AS enrolid, cm.userid,ue.status
              FROM mdl_cohort_members cm
              JOIN mdl_enrol e ON (e.customint1 = cm.cohortid AND e.enrol = 'cohort' AND e.status = :enrolstatus AND e.courseid = :courseid)
              JOIN mdl_user u ON (u.id = cm.userid AND u.deleted = 0)
         LEFT JOIN mdl_user_enrolments ue ON (ue.enrolid = e.id AND ue.userid = cm.userid) JOIN mdl_local_program_users lpu ON (cm.userid = lpu.userid AND lpu.levelids = $data->levelid AND lpu.userid = $data->userid)
             WHERE ue.id IS NULL OR ue.status = :suspended";
        $params['suspended'] = 0;
    } else{
        // $onecourse = $data->id ? "AND e.courseid = :courseid" : "";
        $sql = "SELECT cm.userid, e.id AS enrolid, ue.status
              FROM {cohort_members} cm
              JOIN {enrol} e ON (e.customint1 = cm.cohortid AND e.enrol = 'cohort' AND e.status = :enrolstatus AND e.courseid = :courseid)
