...
function xmldb_attendance_upgrade($oldversion=0) {
    ...
    if ($oldversion < 2022051305.01) {

        // Define table ratings to be created.
        ...
        if (!$dbman->table_exists($table)) {
            ...
            $table->add_field('ratedby', XMLDB_TYPE_INTEGER, '10', null, null, null, null);
            ...
            // Conditionally launch create table for ratings.
            $dbman->create_table($table);
        }
        upgrade_mod_savepoint(true, 2022051305.01, 'attendance');
    }

    /*Dipanshu code starts: To delete the "Excused" record from DB*/
    if ($oldversion < 2022051305.05) {
        $exists = $DB->record_exists('attendance_statuses', array('acronym' => 'E', 'description' => 'Excused'));
        if($exists) {
            $DB->delete_records('attendance_statuses', array('acronym' => 'E', 'description' => 'Excused'));
        }
        upgrade_plugin_savepoint(true, 2022051305.05,'', 'attendance');
    }
    /*Dipanshu code ends: To delete the "Excused" record from DB*/

     if ($oldversion < 2022051305.07) {
        $table = new xmldb_table('attendance_sessions');
        $field = new xmldb_field('teacherid', XMLDB_TYPE_INTEGER, '10', null, null, null, null);
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        upgrade_plugin_savepoint(true, 2022051305.07,'', 'attendance');
    }

    if ($oldversion < 2022051305.11) {
        $table = new xmldb_table('attendance_sessions');
        if ($dbman->table_exists($table)) {
            $field = new xmldb_field('sessionname', XMLDB_TYPE_CHAR, '255', null, null, null, 'null', 'attendanceid');
            if(!$dbman->field_exists($table, $field)){
                $dbman->add_field($table, $field);
            }
            upgrade_plugin_savepoint(true, 2022051305.11, '', 'attendance');
        }
    }

    if ($oldversion < 2022051305.13) {
        $table = new xmldb_table('attendance_sessions');
        ...
        if ($dbman->table_exists($table)) {
            $field = new xmldb_field('description', XMLDB_TYPE_CHAR, '45', null, null, null, null, 'timemodified');
            if(!$dbman->field_exists($table, $field)){
                $dbman->add_field($table, $field);
            }
            upgrade_plugin_savepoint(true, 2022051305.13, '', 'attendance');
        }
    }

    if ($oldversion < 2022051305.14) {
        $table = new xmldb_table('attendance_sessions');
        $field = new xmldb_field('courseid', XMLDB_TYPE_INTEGER, '10', null, null, null, null, 'sessionname');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        upgrade_plugin_savepoint(true, 2022051305.14,'', 'attendance');
    }

    if ($oldversion < 2022051305.15) {
        $table = new xmldb_table('attendance_sessions');
        $field = new xmldb_field('slotid', XMLDB_TYPE_INTEGER, '10', null, null, null, null, null);
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        upgrade_plugin_savepoint(true, 2022051305.15,'', 'attendance');
    }

    if ($oldversion < 2022051305.17) {
        $table = new xmldb_table('attendance_sessions');
        $field = new xmldb_field('session_type', XMLDB_TYPE_INTEGER, '10', null, null, null, null, 'attendanceid');

        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        upgrade_plugin_savepoint(true, 2022051305.17, '', 'attendance');
    }
    if ($oldversion < 2022051305.18) {
        $table = new xmldb_table('attendance_sessions');
        $field = new xmldb_field('building', XMLDB_TYPE_CHAR, '255', null, null, null, null, null);
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        $field = new xmldb_field('room', XMLDB_TYPE_CHAR, '255', null, null, null, null, null);
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        upgrade_plugin_savepoint(true, 2022051305.18, '', 'attendance');
    }
    if ($oldversion < 2022051305.19) {
        $table = new xmldb_table('attendance_sessions');
        $field = new xmldb_field('batch_group', XMLDB_TYPE_INTEGER, '10', null, null, null, 0, 'courseid');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        upgrade_plugin_savepoint(true, 2022051305.19, '', 'attendance');
    }
    if ($oldversion < 2022051305.23) {
        $table = new xmldb_table('attendance_sessions');
        $field = new xmldb_field('building');
        ...
        $field = new xmldb_field('building', XMLDB_TYPE_INTEGER, '10', null, null, null, 0, null);
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        $field = new xmldb_field('room', XMLDB_TYPE_INTEGER, '10', null, null, null, 0, null);
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        upgrade_plugin_savepoint(true, 2022051305.23, '', 'attendance');
    }
    if ($oldversion < 2022051305.24) {
        $table = new xmldb_table('attendance_sessions');
        $field = new xmldb_field('sessedate', XMLDB_TYPE_INTEGER, '10', null, null, null, 0, 'sessdate');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        upgrade_plugin_savepoint(true, 2022051305.24, '', 'attendance');
    }
    if ($oldversion < 2022051305.25) {
        $table = new xmldb_table('attendance_sessions');
        $field = new xmldb_field('batch_group', XMLDB_TYPE_INTEGER, '10', null, null, null, 0, 'courseid');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        upgrade_plugin_savepoint(true, 2022051305.25, '', 'attendance');
    }
