...
class renderer extends plugin_renderer_base {
    ...
    private function construct_take_controls(take_data $takedata) {
        ...
        if (!empty($takedata->pageparams->page) && $takedata->pageparams->page && $totalusers && $usersperpage) {
            ...
            if ($takedata->pageparams->page > 1) {
                $controls .= html_writer::link($takedata->url(array('batch_group' => $batchgroup, 'page' => $takedata->pageparams->page - 1)),
                                                              $this->output->larrow());
            }
            $a = new stdClass();
        ...
        $select = new single_select($takedata->url(array('batch_group' => $batchgroup)), 'viewmode', $options, $takedata->pageparams->viewmode, null);
        ...
        if ($takedata->pageparams->viewmode == mod_attendance_take_page_params::SORTED_GRID) {
            ...
            $select = new single_select($takedata->url(array('batch_group' => $batchgroup)), 'gridcols', $options, $takedata->pageparams->gridcols, null);
            $select->class = 'singleselect inline';
            $controls .= $this->output->render($select);
        }
        if (/*isset($takedata->sessions4copy) && */count($takedata->sessions4copy) > 0) {
            ...
            $select = new single_select($takedata->url(array('batch_group' => $batchgroup), array('copyfrom')), 'copyfrom', $options);
    ...
    private function construct_take_user_controls(take_data $takedata, $user) {
        ...
        if ($user->enrolmentend and $user->enrolmentend < $takedata->sessioninfo->sessdate) {
            ...
            $colors = array('P' => 'green', 'A' => 'red', 'L' => 'orange', /*'E' => 'brown'*/);
            $class = '';
            $status = 'P';
            ...
            $celldata['text'][] = html_writer::tag('span', $status, array('class' => 'attendancestatus'));
