...
class addsession extends moodleform {
    ...
    public function definition() {
        ...
        switch ($groupmode) {
            ...
            $mform->addElement('text', 'sessionname', get_string('sessionname', 'attendance'));
        ...
        if (!empty($studentscanmark)) {
            ...
            // $mform->addgroup($mgroup, 'studentmarking', get_string('studentmarking', 'attendance'));
            // $mform->setDefault('studentmarking', 0);
        ...
        //     $mform->setDefault('studentscanmark', );

            // if (isset($pluginconfig->calendarevent_default)) {
            //     $mform->setDefault('studentscanmark', 'checked');
            // }
        // }
        //  else {
        ...
        //     $mform->addElement('checkbox', 'includeqrcode', '', get_string('includeqrcode', 'attendance'));
        //     $mform->createElement('includeqrcode', 'rotateqrcode', 'checked');
    ...
    public function validation($data, $files) {
        ...
        $existedsessionsql = "SELECT ats.id, ats.attendanceid, ats.sessdate, ats.duration
                                ...
                                AND ats.teacherid = :userid";
        $existedsession = $DB->record_exists_sql($existedsessionsql, array('course' => $cm->course, 'attendance' => $cm->instance, 'sessionstart' => $sessstart, 'sessonstart' => $sessstart, 'sessend' => $sessend, 'sessnd' => $sessend, 'userid' => $usrid));
        if ($existedsession) {
        ...
        $existedsessnothercrsql = "SELECT ats.id, ats.attendanceid, ats.sessdate, ats.duration
                                ...
                                AND ats.teacherid = :usrid";
        $existedsessnothercr = $DB->record_exists_sql($existedsessnothercrsql, array('course' => $cm->course, 'attendance' => $cm->instance, 'sessionstrt' => $sessstart, 'sessonstt' => $sessstart, 'ssend' => $sessend, 'sessndd' => $sessend, 'usrid' => $usrid));
