 ...
 */
 ...
 class notification{
	...
	public function get_notification_strings($emailtype){
		switch($emailtype){
			case 'external_user_register':
                $strings = "[user_name], [user_email]";   
                break;
		}
	}
	public function users_notification($emailtype, $touser, $fromuser, $userinstance){
        if($notification = $this->get_existing_notification($userinstance, $emailtype)){
            $this->send_users_notification($userinstance, $touser, $fromuser, $emailtype, $notification);
        }
    }
    public function get_existing_notification($userinstance, $emailtype){
        ...
        $notification_typesql = "SELECT lni.* FROM {local_notification_info} AS lni 
            JOIN {local_notification_type} AS lnt 
            ON lnt.id=lni.notificationid
            WHERE concat(',',lni.moduleid,',') LIKE concat('%,',:moduleid,',%') AND lnt.shortname LIKE :emailtype 
            AND lni.active=1 ";
    ...
    public function send_users_notification($userinstance, $touser, $fromuser, $emailtype, $notification){
        global $CFG;
