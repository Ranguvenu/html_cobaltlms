...
class local_employees_external extends external_api {

        /**
     * Describes the parameters for submit_create_group_form webservice.
     * @return external_function_parameters
     */
    public static function submit_creates_employees_form_parameters() {
        return new external_function_parameters(
            array(
                'id' => new external_value(PARAM_INT, 'userid', 0),
                'contextid' => new external_value(PARAM_INT, 'The context id for the evaluation', 0),
                'jsonformdata' => new external_value(PARAM_RAW, 'The data from the create group form, encoded as a json array', 0),
     ...
     * @return void
     */
    public function submit_creates_employees_form($id=false, $contextid, $jsonformdata, $form_status){
    ...
    global $PAGE, $CFG, $DB;
        ...
        $warnings = array();
        $mform = new local_employees\forms\create_employees(null, array('form_status' => $form_status,'id' => $data['id'],'org'=>$data['open_costcenterid'],'dept'=>$data['department']), 'post', '', null, true, $data);
        $validateddata = $mform->get_data();
        $userlib = new local_employees\functions\employees();
        if($validateddata){
            $costcenterdata = $DB->get_record('local_costcenter', array('id'=>$validateddata->open_costcenterid));
            $validateddata->open_external = ($costcenterdata->open_external ? $costcenterdata->open_external : 0);
            ...
            if($validateddata->id > 0){
                ...
                $empid->id++;
                $validateddata->employee_id = $validateddata->firstname.$validateddata->lastname.$empid->id;
            ...
            // Generate a warning.
            throw new moodle_exception(get_string('error_in_creation','local_employees'));
        }
        $return = array(
            'id' => $uid,
     ...
     * @since Moodle 3.0
     */
    public static function submit_creates_employees_form_returns() {
    ...
    public function trash_employees_parameters(){
        return new external_function_parameters(
            array(
                'id' => new external_value(PARAM_INT, 'userid', 0),
    ...
    public function trash_employees($id,$contextid){
        global $DB;
    ...
    public function trash_employees_returns(){
        return new external_value(PARAM_BOOL, 'return');
    }

    public function suspend_employees_parameters(){
        return new external_function_parameters(
            array(
                'id' => new external_value(PARAM_INT, 'userid', 0),
                'contextid' => new external_value(PARAM_INT, 'The context id for the evaluation', false)
    ...
    public function suspend_employees($id,$contextid){
        global $DB;
        ...
        if($user){
            ...
            $localclassroom_plugin_exist = $core_component::get_plugin_directory('local', 'classroom');
            if(!empty($localclassroom_plugin_exist)){
                if (method_exists(new \local_classroom\classroom(), 'trash_suspend_employees_remove_classrooms')) {

                        (new \local_classroom\classroom)->trash_suspend_employees_remove_classrooms($user->id);
                    }
            }
            $return = TRUE;
        }else {
            throw new moodle_exception(get_string('error_in_inactivating','local_employees'));
            $return = FALSE;
        }
        return $return;
    }
    public function suspend_employees_returns(){
        return new external_value(PARAM_BOOL, 'return');
    }
    ...
    public function get_departments_list($costcenterid, $contextid){
        ...
        self::validate_context($context);
        global $DB;
        $selectarray = array(0 => get_string('selectdepartment', 'local_employees'));
    ...
    public function get_supervisors_list($costcenterid, $contextid){
        ...
        self::validate_context($context);
        global $DB;
            $selectarray = array(0 => get_string('selectsupervisor', 'local_employees'));
        if($params['costcenterid'] > 0){
            try{
     ...
     * @return external_function_parameters.
     */
    public static function manage_employees_view_parameters() {
        return new external_function_parameters([
     ...
     */
    public static function manage_employees_view(
        $options,
        $dataoptions,
        $offset = 0,
        $limit = 0,
        ...
        require_once($CFG->dirroot . '/local/employees/lib.php');
        require_login();
        $PAGE->set_url('/local/employees/index.php', array());
        $PAGE->set_context($contextid);
        // Parameter validation.
        $params = self::validate_parameters(
            self::manage_employees_view_parameters(),
            [
                'options' => $options,
        ...
        $totalusers = manage_employees_count($stable,$filtervalues);
        ...
        $stable->start = $offset;
        $stable->length = $limit;
      $data = manage_employees_content($stable,$totalusers);
        return [
     ...
     * Returns description of method result value.
     */
    public static function  manage_employees_view_returns() {
            ...
            'records' => new external_multiple_structure(
                            new external_single_structure(
                                array(
                                    ...
                                    'open_type' => new external_value(PARAM_RAW, 'role of the user', VALUE_OPTIONAL),
                                    'orgstring' => new external_value(PARAM_RAW, 'org string of the user', VALUE_OPTIONAL),

                                    'deptstring' => new external_value(PARAM_RAW, 'dept string ', VALUE_OPTIONAL),
                                    'programstring' => new external_value(PARAM_RAW, 'programstring', VALUE_OPTIONAL),
                                    'batchinfo' => new external_value(PARAM_RAW, 'batchinfo', VALUE_OPTIONAL),
                                    ...
                                    'procount' => new external_value(PARAM_RAW, 'procount', VALUE_OPTIONAL),
                                    ...
                                    'programid' => new external_value(PARAM_RAW, 'programid', VALUE_OPTIONAL),
                                    'studentcount' => new external_value(PARAM_RAW, 'studentcount', VALUE_OPTIONAL),
                                    'percentage' => new external_value(PARAM_RAW, 'percentage', VALUE_OPTIONAL),
                                    'cid' => new external_value(PARAM_RAW, 'cid', VALUE_OPTIONAL),
                                    'programcount' => new external_value(PARAM_RAW, 'programcount', VALUE_OPTIONAL),
                                    'fullemail' => new external_value(PARAM_RAW, 'fullemail', VALUE_OPTIONAL),
                                    'delteacher' => new external_value(PARAM_RAW, 'delteacher', VALUE_OPTIONAL),
                                    'firstlabelstring' => new external_value(PARAM_RAW, 'firstlabelstring', VALUE_OPTIONAL),
                                )
                            )
                        )
        ]);
    ...
    public static function profiledata(
        ...
        require_once($CFG->dirroot . '/local/employees/lib.php');
        require_login();
        $PAGE->set_url('/local/employees/profile.php', array());
        $PAGE->set_context($contextid);
        // Parameter validation.
        $params = self::validate_parameters(
        ...
        $userclass = '\local_'.$decodedata->moduletype.'\local\employees';
    ...
    public static function pending_activities($events = array(), $options = array()) {
        ...
        $mycourses = \local_courses\local\employees::enrol_get_employees_courses($USER->id, false, false, 0, 5, true);
    ...
    public static function get_course_grades($userid = 0, $source = 'mobile') {
        ...
        require_once($CFG->dirroot . '/local/employees/grades.php');
        $warnings = array();
        ...
        $gpr = new employees_plugin_return(array('type' => 'report', 'plugin' => 'overview', 'courseid' => $course->id,
                                        'userid' => $userid));
        $report = new employees_grades($userid, $gpr, $context);
        $coursesgrades = $report->setup_courses_data(true);
        if($source = 'mobile'){
            $secured_courses = $DB->get_fieldset_sql("SELECT c.id FROM {course} AS c WHERE c.id>1 AND c.open_securecourse = :open_securecourse ", array('open_securecourse' => 1));
        }else{
            $secured_courses = [];
        }
    ...
    public static function submit_profile_update_form_parameters() {
        return new external_function_parameters(
            array(
                'id' => new external_value(PARAM_INT, 'userid', 0),
                'contextid' => new external_value(PARAM_INT, 'The context id for the user', false),
    ...
    /**
     * [submit_profile_update_form description]
     * @param  [integer] $contextid
     * @param  [string] $jsonformdata
     * @return void
     */
    public function submit_profile_update_form($id, $contextid, $jsonformdata, $form_status){
        global $PAGE, $CFG, $DB, $SESSION;
        require_once($CFG->dirroot . '/local/employees/lib.php');
        // We always must pass webservice params through validate_parameters.
        ...
        parse_str($serialiseddata, $data);
        $warnings = array();
        $mform = new local_employees\forms\profile_form(null, array('form_status' => $form_status,'id' => $data['id'],'org'=>$data['open_costcenterid'],'dept'=>$data['department']), 'post', '', null, true, $data);
        $validateddata = $mform->get_data();
        $userlib = new local_employees\functions\employees();
        if($validateddata){
            if($validateddata->id > 0){
                $uid = $userlib->update_user_profile($validateddata);
            }
            $formheaders = array_keys($mform->formstatus);
            $next = $form_status + 1;
            $nextform = array_key_exists($next, $formheaders);
     ...
     * @since Moodle 3.0
     */
    public static function submit_profile_update_form_returns() {
        return new external_single_structure(array(
            'id' => new external_value(PARAM_INT, 'Userid'),
            'form_status' => new external_value(PARAM_INT, 'form_status'),
        ));
