...
use events;
use context_user;
use local_employees\functions\employeeslibfunctions as employeeslib;
use local_costcenter\functions\userlibfunctions as costcenterlib;

class create_employees extends moodleform {
	public $formstatus;
	public function __construct($action = null, $customdata = null, $method = 'post', $target = '', $attributes = null, $editable = true, $formdata = null) {
	 	...
	 	$this->formstatus = array(
	 		'generaldetails' => get_string('generaldetails', 'local_employees'),
			'contactdetails' => get_string('contactdetails', 'local_employees'),
			);
    ...
    public function definition() {
        ...
        $costid = $DB->get_field('user','open_costcenterid', array('id' =>$id));
        $dept = $DB->get_field('user','open_departmentid', array('id' =>$id));
        $subdep = $DB->get_field('user','open_subdepartment', array('id' =>$id));
        $usererc = $DB->get_record('user', ['id' => $id]);
        ...
        if($form_status == 0){
            ...
            if ($id > 0 && $user_exist) {
                $rolenamesql = "SELECT r.id, r.name FROM {role} r
                                                JOIN {role_assignments} ra ON ra.roleid = r.id
                                                WHERE ra.userid = $id";
                    $rolename = $DB->get_record_sql($rolenamesql);
                    if(!$rolename){
                            $rolename = $DB->get_record_sql("SELECT r.id, r.name FROM {role} r
                                            JOIN {user} u ON u.roleid = r.id WHERE u.id = $id");
                        }        
                     $mform->addElement('static', 'rolename', get_string('emprole', 'local_employees'), $rolename->name);
                     $mform->addElement('hidden', 'roleid', $rolename->id);
                     $mform->setDefault('roleid', $rolename->id);
                ...
                $getdepartmentelements = costcenterlib::org_hierarchy($mform, $id, $context, $mformajax, 'employee', 'employeeclass');
            }
            // $getdepartmentelements = costcenterlib::org_hierarchy($mform, $id, $context, $mformajax, 'employee', 'employeeclass');

            $mform->addElement('text', 'email', get_string('email', 'local_employees'), $readonly);
            ...
            $mform->addElement('text', 'firstname', get_string('firstname', 'local_employees'), $readonly);
            $mform->addRule('firstname', get_string('errorfirstname', 'local_employees'), 'required', null, 'client');
            $mform->setType('firstname', PARAM_RAW);
            
            $mform->addElement('text', 'lastname', get_string('lastname', 'local_employees'), $readonly);
            $mform->addRule('lastname', get_string('errorlastname', 'local_employees'), 'required', null, 'client');
            $mform->setType('lastname', PARAM_RAW);

            $mform->addElement('text', 'city', get_string('city','local_employees'),'maxlength="120" size="20"');
        ...
        if (!empty($CFG->country)) {
            ...
            $mform->addElement('textarea', 'address', get_string('address','local_users'), 'maxlength="255" size="20"');
	        ...
	        $mform->addElement('filepicker', 'imagefile', get_string('profile_picture','local_employees'), null, array('accepted_types' => array('.jpg', '.jpeg', '.png')));
	        $mform->addHelpButton('imagefile', 'profile_picture','local_employees');
        ...
        if($form_status == 0){// as these fields are in only form part 1(form_status=0)
	        ...
	        if(!$data['createpassword']){
		        if (!empty($data['password']) ) {
	            	...
	            	$errors['password'] = get_string('passwordrequired', 'local_employees');
	            }
	        }
            if (!empty($data['phone1'])) {
                if (strlen($data['phone1']) !== 10) {
                    $errors['phone1'] = get_string('errormobiledigits', 'local_employees');
                }
                if (!is_numeric($data['phone1'])) {
                    $errors['phone1'] = get_string('acceptedtype', 'local_employees');
                }
            ...
            if($data['roleid'] == 0){
                $errors['roleid'] = get_string('errorrole', 'local_costcenter');
            }

            $roleshortname = $DB->get_field('role', 'shortname', ['id' => $data['roleid']]);
            if($id > 0){
                // $user_exist = $DB->record_exists('local_program_users', ['userid' => $id]);
                $user_exist = $DB->get_fieldset_sql("SELECT c.id as courseid
                FROM {user} u
            ...
            // print_r($roleshortname);exit;
        // if(is_siteadmin() || has_capability('local/costcenter::manage_multiorganizations', $systemcontext) 
        //     || has_capability('local/costcenter::manage_ownorganizations', $systemcontext) || has_capability('local/costcenter::manage_owndepartments', $systemcontext) ){
            if(!$user_exist){
                if($roleshortname == 'editingteacher'){
                    // print_r($data);exit;
                    // open_departmentid
                    // open_subdepartment
                    if($data['open_departmentid'] == 0){
                        $errors['open_departmentid'] = get_string('teachercannotbeAlldept', 'local_employees', $labelstring->secondlevel);
                    }
                    if($data['open_subdepartment'] == 0){
                        $errors['open_subdepartment'] = get_string('teachercannotbeAllsub', 'local_employees', $labelstring->thirdlevel);
                    }
                }
                if($roleshortname == 'orgadmin'){
                    if($data['open_departmentid'] != 0){
                        $errors['open_departmentid'] = get_string('orgadmincannotbedept', 'local_employees', );
                    }
                    if($data['open_subdepartment'] != 0){
                        $errors['open_subdepartment'] = get_string('orgadmincannotbesub', 'local_employees');
                    }
                }
                if($roleshortname == 'collegeadmin'){
                    if($data['open_departmentid'] == 0){
                        $errors['open_departmentid'] = get_string('collegeadmincannotbedept', 'local_employees', $labelstring->secondlevel);
                    }
                    // if($data['open_subdepartment'] != 0){
                    //     $errors['open_subdepartment'] = get_string('collegeadmincannotbesub', 'local_employees');
                    // }
                }
                if($roleshortname == 'departmentadmin'){
                    if($data['open_departmentid'] == 0){
                        $errors['open_departmentid'] = get_string('departmentadmincannotbedept', 'local_employees', $labelstring->secondlevel);
                    }
                    if($data['open_subdepartment'] == 0){
                        $errors['open_subdepartment'] = get_string('departmentadmincannotbesub', 'local_employees', $labelstring->thirdlevel);
                    }
                }
            }
