...
class extsignup_form extends moodleform {
	...
	public function __construct($action = null, $customdata = null, $method = 'post', $target = '', $attributes = null, $editable = true, $formdata = null) {
	 	...
	 	$this->formstatus = array(
			...
			'otherdetails' => get_string('otherdetails', 'local_employees'),
    ...
    public function definition() {
        ...
        $namefields = employeesedit_get_required_name_fields();
        foreach ($namefields as $field) {
            $mform->addElement('text', $field, get_string($field,'local_employees'), 'maxlength="100" size="30"');
            $stringid = 'missing' . $field;
            if (!get_string_manager()->string_exists($stringid, 'moodle')) {
                $stringid = 'required';
            }
            $mform->addRule($field, get_string($stringid,'local_employees'), 'required', null, 'client');
        }
        ...
        $this->add_action_buttons(true, get_string('createaccount', 'local_employees'));
    }
    ...
    public function validation($data, $files) {
        ...
        if(empty($result) || empty($dept)){
            $errors['email'] = get_string('domainnotexists', 'local_employees');
        }
