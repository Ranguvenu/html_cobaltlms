...
/**
 * Event observer for local_employees. Dont let other user to view unauthorized users
 */
class local_employees_observer extends \core\event\employees_profile_viewed {
    /**
     * Triggered via user_profile_viewed event.
     *
     * @param \core\event\employees_profile_viewed $event
     */
    public static function employees_profile_viewed(\core\event\employees_profile_viewed $event) {
        ...
        if (($related_userid != $USER->id) AND (!(is_siteadmin() OR has_capability('local/costcenter:manage_multiorganizations',$systemcontext)))) {
            if((has_capability('local/employees:create',$systemcontext))){
                ...
                if ($user_costcenter != $manager_costcenter) {
                    redirect($CFG->wwwroot.'/local/employees/index.php');
                die;
                }
            } else {
                redirect($CFG->wwwroot.'/local/employees/index.php');
    ...
    public static function employees_loggedin(\core\event\employees_loggedin $event) {
        global $DB, $CFG, $USER;
