...
class syncfunctionality {
    ...
    public function main_hrms_frontendform_method($cir, $filecolumns, $formdata) {
        ...
        while ($line = $cir->next()) {
            ...
            $mandatory_fields = ['first_name', 'last_name', 'username', 'organization', 'employee_id', 'email','employee_status'];
            ...
            if (count($existing_user == 1)) {
                ...
                $this->errors[] = get_string('multiple_user', 'local_employees');
            } else {
            ...
            //     $this->errors[] = get_string('password_upload_error', 'local_employees', $strings);
            //     echo '<div class=local_employees_sync_error>'.get_string('password_upload_error', 'local_employees', $strings).'</div>';
            //     $this->errorcount++;
            // }
        ...
        if ($this->data) {
            ...
            echo'<div class="critera_error">'.get_string('filenotavailable', 'local_employees').'</div>';
        }
    ...
    public function get_org_hierarchyid($fieldvalue, $parent) {
        ...
        if ($datal) {
            ...
            echo '<div class=local_employees_sync_error>'.get_string('noorganizationidfound', 'local_employees', $strings).'</div>';
            $this->errors[] = get_string('noorganizationidfound', 'local_employees', $strings);
    ...
    public function mandatory_field_validation($user, $field) {
        ...
        if (empty(trim($user->$field))) {
              ...
              $strings->field = $field;
              $strings->linenumber = $this->excel_line_number;
            echo '<div class=local_employees_sync_error>'.get_string('missing', 'local_employees', $strings).'</div>';
            $this->errors[] = get_string('missing', 'local_employees', $strings);
            $this->mfields[] = $field;
            $this->errorcount++;
        }
    ...
    public function employee_status_validation($excel) {
        ...
        if (array_key_exists('employee_status', $excel)) {
            if (strtolower($excel->employee_status) == 'active') {
                ...
                $strings = new stdClass;
                $strings->line = $this->excel_line_number;
                echo '<div class=local_employees_sync_error>'.get_string('statusvalidation', 'local_employees', $strings).'</div>';
                $this->errors[] = get_string('statusvalidation','local_employees', $strings);
            ...
            $this->errormessage = get_string('columnsarragement_error','local_employees', $excel);
            $this->errorcount++;
        }
    ...
    public function get_subdepartmentid($subdepartmentid, $parentid) {
        ...
        if ($datal) {
            ...
            $strings->line = $this->excel_line_number;
            $warningmessage = get_string('noorsubdepartmentfound','local_employees',$strings);
            $this->errormessage = $warningmessage;
             echo '<div class=local_employees_sync_warning>'.$warningmessage.'</div>';
            $this->warningscount++;
        }
