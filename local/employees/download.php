...
global $CFG, $USER;
$myuser = new local_employees\functions\employees();
$costcenter = new costcenter();
$systemcontext = context_system::instance();
require_login();
if(!has_capability('local/employees:manage',$systemcontext) ){
    print_error('You dont have a permission to view this page.');
}
$context = context_system::instance();
$extracolumns = get_extra_user_fields($context);
$columns = array_merge(array('firstname', 'lastname'), $extracolumns, array('username','idnumber','costcenter', 'role', 'lastaccess'));
...
$baseurl = new moodle_url('/local/employees/index.php', array('sort' => $sort, 'dir' => $dir, 'perpage' => $perpage));
if (!$users) {
    $match = array();
    ...
    $table->head[] = get_string('login','local_employees');
    ...
    foreach ($users as $user) {
        ...
        if ($user->suspended) {
            foreach ($line as $k => $v) {
                $line[$k] = html_writer::tag('span', $v, array('class' => 'employeesuspended'));
            }
        }
        if (has_capability('local/employees:manage', $systemcontext))
        $data[] = $line;
