...
require_once(dirname(__FILE__) . '/../../config.php');
    ...
    $PAGE->navbar->add(get_string('uploadusers', 'local_employees'), new moodle_url('/local/employees/upload.php'));
    ...
    echo '<div style="float:right;"><a href="upload.php"><button>' . get_string('back_upload', 'local_employees') . '</button></a></div>';
    }
    if (isset($CFG->allowframembedding) and ! $CFG->allowframembedding) {
    ...
    echo get_string('help_1', 'local_employees');
    $select = new single_select(new moodle_url('#'), 'proid', $countries, null, '');
    $select->set_label('');
    echo $OUTPUT->render($select);
    echo get_string('help_2', 'local_employees');
    echo $OUTPUT->footer();
