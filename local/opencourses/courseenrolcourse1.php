...
if ($view == 'ajax') {
  ...
  $select_from_users = course_enrolled_users2($type, $course_id, $options, false, $offset1 =- 1, $perpage = 50, $lastitem);
...
if($roleid == 5){
$PAGE->set_url('/local/opencourses/courseenrol.php', array('enrolid' => $instance->id,'roleid'=>$roleid,'id' => $course_id, 'programid'=>0,'costcenterid'=>$costcenteridstd, 'subdepartmentid'=>$subdepartmentid, 'batchid'=>0));
}
...
// $filterlist = get_filterslist();
    if (is_siteadmin() || has_capability('local/costcenter:manage_multiorganizations', $context)) {
      ...
      $filterlist = array('departments','subdepartment','empemail');
    } else if (!is_siteadmin() && has_capability('local/costcenter:manage_owndepartments', $context )) {
      $filterlist = array('subdepartment','empemail');
    } else {
      $filterlist = array('subdepartment','empemail');
    }
  }
  ...
  if($roleid == 5){
    if (is_siteadmin() || has_capability('local/costcenter:manage_multiorganizations', $context)) {
      ...
      $filterlist = array('departments','subdepartment','email');
    } else if (!is_siteadmin() && has_capability('local/costcenter:manage_owndepartments', $context )) {
  ...
  $mform = new moocfilters_form($PAGE->url, array('filterlist' => $filterlist, 'enrolid' => $enrolid, 'courseid' => $course_id, 'filterparams' => $filterparams, 'action' => 'user_enrolment'));
...
echo $OUTPUT->footer();
  ...
  $select_to_users = course_enrolled_users2('add', $course_id, $options, false, $offset =- 1, $perpage = 50);
  $select_to_users_total = course_enrolled_users2('add', $course_id, $options, true, $offset1 =- 1, $perpage =- 1);

  $select_from_users = course_enrolled_users2('remove', $course_id, $options, false, $offset1=-1, $perpage = 50);
