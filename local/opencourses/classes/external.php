...
use \local_opencourses\action\insert as insert;
use \local_opencourses\local\general_lib as general_lib;
use \local_opencourses\form\custom_courseevidence_form as custom_courseevidence_form;
...
class local_opencourses_external extends external_api {
  ...
  global $DB, $CFG, $USER;
        ...
        $mform = new custom_opencourse_form(null, array('form_status' => $form_status, 'courseid' => $data['id']), 'post', '', null, true, $data);
        ...
        if ($validateddata) {
            ...
            if ($validateddata->id <= 0) {
                $validateddata->open_identifiedas = 6;//$validateddata->open_identifiedas;
...
//'enrolid' => new external_value(PARAM_INT, 'manual enrol id for the course'),
  ...
  */
   ...
   * @return external_function_parameters
  */
  public static function opencourses_view_parameters() {
    return new external_function_parameters([
   ...
   */
  public static function opencourses_view($options, $dataoptions, $offset = 0, $limit = 0, $contextid, $filterdata) {
    global $DB, $PAGE;
    require_login();

    $PAGE->set_url('/local/opencourses/opencourses.php', array());
 ...
 $labelstring = get_config('local_costcenter');
    ...
    $params = self::validate_parameters(
        self::opencourses_view_parameters(),
        [
            'options' => $options,
    ...
    $limit = $params['limit'];
    $display_url = new moodle_url('/local/opencourses/opencourses.php?formattype=' . $formattype_url);
    ...
    $data = get_listof_opencourses($stable, $filtervalues);
  ...
  public static function opencourses_view_returns() {
                      ...
                      'hascourses' => new external_multiple_structure(
                          new external_single_structure(
                              array(
                                  ...
                                  'student_enrolments' => new external_value(PARAM_RAW, 'student_enrolments', VALUE_OPTIONAL),
                                  'faculty_enrolments' => new external_value(PARAM_RAW, 'faculty_enrolments', VALUE_OPTIONAL),
