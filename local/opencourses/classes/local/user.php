...
class user{
    ...
    public function user_modulewise_content($id, $start =0, $limit=5){
      ...
      foreach($usercourses['data'] as $course){
          ...
          $statusid = $DB->get_field_sql("SELECT id FROM {course_completions} WHERE course =:courseid AND userid =:userid AND timecompleted IS NOT NULL ", array('userid' => $id, 'courseid' => $course->id ));
