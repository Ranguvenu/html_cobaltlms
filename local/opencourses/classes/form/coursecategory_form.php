...
class coursecategory_form extends moodleform {
    ...
    public function definition() {
        ...
        $mform->addRule('name', get_string('entercategory','local_courses'), 'required', null);
        $mform->setType('name', PARAM_TEXT);

        $mform->addElement('text', 'idnumber', get_string('categorycode','local_courses'), 'maxlength="100" size="10"');
        $mform->addRule('idnumber', get_string('entercategorycode','local_courses'), 'required', null);
    ...
    public function validation($data, $files) {
        ...
        $parent = $data['parent'];
        $sql = "SELECT id from {course_categories} WHERE idnumber LIKE '{$idnumber}' ";
        if($data['id']){
            $sql .= " AND id <> {$data['id']} ";
        }
