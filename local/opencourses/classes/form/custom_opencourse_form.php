...
namespace local_opencourses\form;
...
class custom_opencourse_form extends moodleform {
    protected $course;
    protected $context;
    public $formstatus;
    public function __construct($action = null, $customdata = null, $method = 'post', $target = '', 
        ...
        $this->formstatus = array(
            'manage_opencourse' => get_string('manage_opencourse', 'local_opencourses'),
...
// Sandeep Category For future refrence (do not remove commented code). - Starts //
    ...
    function validation($data, $files) {
        ...
        if (!empty($data['shortname']) && strrpos($data['shortname'], ' ') == true) {
            $errors['shortname'] = get_string('spacesnotallowed', 'local_opencourses');
        }
