...
if($view == 'ajax'){
  ...
  $select_from_users=mooccourse_enrolled_users1($type, $course_id,$options,false,$offset1=-1,$perpage=50,$lastitem);
...
$PAGE->set_url('/local/opencourses/courseenrol.php', array('id'=>$course_id,'enrolid'=>$instance->id,'costcenter'=>$organization, 'roleid'=>$roleid));
$PAGE->set_pagelayout('standard');
$PAGE->navbar->add(get_string('manage_courses','local_courses'),new moodle_url('/local/opencourses/index.php'));
...
if(!$add&&!$remove){
  ...
  // $PAGE->set_heading('Enroll Students to "'.$course->fullname.'" Course');
    $PAGE->set_heading(get_string('studentmooccourseenrollments', 'local_opencourses', $course->fullname));
  }elseif($rolename == 'faculty'){
    $PAGE->set_heading(get_string('facultymooccourseenrollments', 'local_opencourses', $course->fullname));
  }else{
...
if ($course) {
  ...
  // print_object($filterdata);die;
    ...
    //print_object($filterdata);exit;
     ...
     $organizations = !empty($filterdata->organizations) ? implode(',', $filterdata->organizations) : null;
    $department = !empty($filterdata->departments) ? implode(',', $filterdata->departments) : null;

    // if($filterdata->saveanddisplay == 'Apply'){
    //   echo "dfdf";
    //   die;
    ...
    $emailid = !empty($filterdata->emailid) ? $filterdata->emailid : null;
    ...
    // Create the user selector objects.
    $options = array('context' => $context->id, 'courseid' => $course_id, 'organization' => $organization, 'organizations' => $organizations,'department' => $department, 'emailid' => $emailid, 'idnumber' => $idnumber, 'uname' => $uname, 'roleid' => $roleid, 'groups' => $groups);
        // print_object($options);exit;
  $dataobj = $course_id;
  $fromuserid = $USER->id;
  // print_object($options);die;
    ...
    $type = 'course_enrol';
        ...
        if (!empty($userstoassign)) {
            ...
            $progress = 0;
            $progressbar = new \core\progress\display_if_slow(get_string('enrollusers', 'local_opencourses',$course->fullname));
            ...
            if($rolename == 'student'){
                $enrollsuccessmsg = get_string('enrolluserssuccess', 'local_opemncourses',$result);
            }else{
                $enrollsuccessmsg = get_string('enrollfacultysuccess', 'local_opencourses',$result);
            }
    ...
    if ( $remove&& confirm_sesskey()) {
        ...
        if($submit_value=="Remove_All_Users"){
          $options =json_decode($_REQUEST["options"],false);
             $userstounassign = array_flip(mooccourse_enrolled_users1('remove',$course_id,(array)$options,false,$offset1=-1,$perpage=-1));
        }else{
            $userstounassign = $remove;
        }
        if (!empty($userstounassign)) {
            $progress = 0;
            $progressbar = new \core\progress\display_if_slow(get_string('un_enrollusers', 'local_opencourses',$course->fullname));
            $progressbar->start_html();
            ...
            if($rolename == 'student'){
                $unenrollsuccessmsg = get_string('unenrolluserssuccess', 'local_opencourses',$result);
            }else{
                $unenrollsuccessmsg = get_string('unenrollfacultysuccess', 'local_opencourses',$result);
            }
    ...
    $select_to_users = mooccourse_enrolled_users1('add', $course_id, $options, false, $offset=-1, $perpage=50);

    $select_to_users_total = mooccourse_enrolled_users1('add', $course_id, $options, true, $offset1=-1, $perpage=-1);
 
    $select_from_users = mooccourse_enrolled_users1('remove', $course_id, $options, false, $offset1=-1, $perpage=50);
    $select_from_users_total = mooccourse_enrolled_users1('remove', $course_id, $options, true, $offset1=-1, $perpage=-1);
  ...
  $content.='<div class="box3 col-12 col-md-2 actions pull-left">
              <button type="submit" class="custom_btn btn remove btn-default" disabled="disabled" title="Remove Selected Students" name="submit_value" id="user_unassign_all">
                '.get_string('remove_selected_users', 'local_courses').'
              </button>
            </form>';
  $content.='<form  method="post" name="form_name" id="user_un_assign" class="form_class" >
              <button type="submit" class="custom_btn btn move btn-default" disabled="disabled" title="Add Selected Students" name="submit_value" id="user_assign_all" >
...
echo '<a class="btn-link btn-sm d-flex align-items-center filter_btn" href="javascript:void(0);" data-toggle="collapse" data-target="#local_mooccourses-filter_collapse" aria-expanded="false" aria-controls="local_mooccourses-filter_collapse">
        <span class="filter mr-2">Filters</span>
            <i class="m-0 fa fa-sliders fa-2x" aria-hidden="true"></i>
          </a>';
    echo  '<div class="mt-2 mb-2 collapse '.$show.'" id="local_mooccourses-filter_collapse">
              ...
              if(count_selected_list<total_users){  
                   //alert('end reached');
                    var selected_list_request = $.ajax({
                        method: 'GET',
                        url: M.cfg.wwwroot + '/local/opencourses/courseenrol.php?options=$myJSON',
