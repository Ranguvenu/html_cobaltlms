...
require_once($CFG->dirroot . '/local/opencourses/upload/course.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class local_uploadcourse_processor1 {
  ...
  $systemcontext = context_system::instance();
        ...
        while ($line = $this->cir->next()) {
           ...
           if (!is_siteadmin() && !has_capability('local/costcenter:manage_multiorganizations', $systemcontext) && !has_capability('local/costcenter:manage_ownorganization', $systemcontext) && has_capability('local/costcenter:manage_owndepartments', $systemcontext)) {
              ...
              $data['open_departmentid'] = $USER->open_departmentid;//$DB->get_field('local_costcenter', 'id', array('shortname' => $USER->open_departmentid));
}else{
    $data['open_departmentid'] = $DB->get_field('local_costcenter', 'id', array('shortname' => $data['College']));
}
            
             $subdepartmentparams = array('shortname' => $data['Department']);
            ...
            $data['open_identifiedas'] = 6;//implode(',', $identify);
