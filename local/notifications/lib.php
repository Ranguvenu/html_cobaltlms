...
function local_notifications_leftmenunode(){
    ...
    if(is_siteadmin() || has_capability('local/costcenter:manage_ownorganization', $systemcontext)){
        $notificationsnode .= html_writer::start_tag('li', array('id'=> 'id_leftmenu_notifications', 'class'=>'pull-left user_nav_div notifications'));
            ...
            $notifications = html_writer::link($notifications_url, '<span class="noti_wht_structure_icon dypatil_cmn_icon icon"></span><span class="user_navigation_link_text">'.get_string('pluginname','local_notifications').'</span>',array('class'=>'user_navigation_link'));
...
function notification_details($tablelimits, $filtervalues){
        ...
        $selectsql = "SELECT ni.id, nt.name, nt.shortname, nt.name, ni.subject, ni.costcenterid, ni.moduleid,
                        lc.fullname as deptname, ni.active";
        $concatsql .= " FROM {local_notification_info} ni
        ...
        if (isset($filtervalues->search_query) && trim($filtervalues->search_query) != '') {
            $filteredquizzes = array_filter(explode(',', $filtervalues->search_query));
            $quizzesarray = array();
            if (!empty($filteredquizzes)) {
                foreach ($filteredquizzes as $key => $value) {
                    $quizzesarray[] = "nt.shortname LIKE '%".trim($value)."%' OR nt.name LIKE '%".trim($value)."%'";
                    $quizzesarray[] = "lc.fullname LIKE '%".trim($value)."%' OR ni.subject LIKE '%".trim($value)."%'";
                }
        ...
        if(!empty($filtervalues->request)){
            ...
            $request = array_filter(explode(',',$filtervalues->request), 'is_numeric');
            $requestsarray = array();
            foreach ($request as $key => $value) {
                $requestsarray[] = " nt.parent_module = '$value' ";
            }
...
function notifications_filter($mform){
    ...
    if ((has_capability('local/notifications:view', context_system::instance()) || is_siteadmin())) {
       $sql = "select id, shortname from {local_notification_type} where parent_module = 0";
       $notificationlist = $DB->get_records_sql($sql);
        foreach($notificationlist AS $list){
            $customrequestlist[$list->id] = ucfirst($list->shortname);
        }
        $requestlist = $customrequestlist; 
    }
    $select = $mform->addElement('autocomplete', 'request', '', $requestlist, array('placeholder' => get_string('compname',  'local_notifications')));
