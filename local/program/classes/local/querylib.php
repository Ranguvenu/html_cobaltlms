...
class querylib {
    ...
    public function get_user_department_trainerslist($service = false, $costcenters = false,
        $trainers = array(), $query = '', $levelinstanceid = 0) {
        global $DB, $USER;
        ...
        $levelinstance = $DB->get_record('local_program_level_courses', ['id' => $levelinstanceid]);

        $coursecontext = \context_course::instance($levelinstance->courseid);
        $params['ctx1'] = $coursecontext->id;
        if (!empty($trainers)) {
        ...
        $params['suspended'] = 0;
        $params['deleted'] = 0;
        $params['roleid'] = $DB->get_field('role', 'id', array('shortname' => 'editingteacher'));
    ...
    public function get_program_institute_rooms($programid) {
     ...
     public function get_program_streamlist() {
        ...
        // added for getting curriculum list in program form
     public function get_program_curriculumlist() {
        global $DB;

        $curriculums = array();
        $curriculumsql = "SELECT id,name FROM {local_curriculum}";
        $curriculumlist = $DB->get_records_sql($curriculumsql);

        foreach($curriculumlist as $curriculum){
            $curriculums[$curriculum->id] = $curriculum->name;
        }
        return $curriculums;
     }

   public function get_program_costcenter() {
        global $DB;

        $costs = array();
        
        $costsql = "SELECT id,fullname FROM {local_costcenter} 
                    WHERE parentid=0";
        $costlist = $DB->get_records_sql($costsql);

        foreach($costlist as $cost){
            $costs[$cost->id] = $cost->fullname;
        }
        return $costs;
    
    }

     //Added for add curriculum  popup from local_curriculum
    public function get_curriculum_programlist($id=0){
         global $DB;
        $param=array();
        $programssql = "SELECT id, name,costcenter
