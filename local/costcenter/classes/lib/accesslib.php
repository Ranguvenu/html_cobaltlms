...
class accesslib
{
    ...
    public static function userpath_match_sql($matchcolumnname,$datatype){
        ...
        if(!is_siteadmin()){
                ...
                // $usercostcenterpaths = $DB->get_records_sql_menu('local_userdata', array('userid' => $USER->id), '', 'id, costcenterpath');

                $sql = "SELECT lc.id, path FROM {local_costcenter} lc JOIN {user} u ON lc.id = u.open_costcenterid WHERE u.id = $USER->id";

                $usercostcenterpaths = $DB->get_records_sql_menu($sql);
