...
class local_location_renderer extends plugin_renderer_base {
    /**
     * Display the avialable locations
     *
     * @return string The text to render
     */
    public function location_view_content($filter = false) {
        global $USER;
        $systemcontext = context_system::instance();
        $cardClass = 'tableformat';
        $options = array(
            'targetID' => 'tm_locations',
            'perPage' => 10,
            'cardClass' => 'col-md-4 col-12',
            'viewType' => 'table'
        );
        $options['methodName'] = 'local_location_view_instance';
        $options['templateName'] = 'local_location/index';
    ...
    /**
     * Display the avialable rooms
     *
     * @return string The text to render
     */
    public function room_view_content($filter = false) {
        global $USER;
        $systemcontext = context_system::instance();
        $cardClass = 'tableformat';
        $options = array(
        ...
        $options['methodName'] = 'local_room_view_instance';
        $options['templateName'] = 'local_location/room';
        $options = json_encode($options);
        $filterdata = json_encode(array());
        $dataoptions = json_encode(array('userid' => $USER->id, 'contextid' => $systemcontext->id));
    ...
    public function display_institutes() {
        ...
        if ((has_capability('local/location:manageinstitute', context_system::instance()) || has_capability('local/location:viewinstitute', context_system::instance())) && ( !is_siteadmin() ) ) {
 ...
 */
     public function display_rooms() {
       ...
       if ((has_capability('local/location:manageroom', context_system::instance()) || has_capability('local/location:viewroom', context_system::instance())) && ( !is_siteadmin() ) ) {
