...
function xmldb_local_location_upgrade($oldversion) {
 ...
 if ($oldversion < 2022101800) {
    ...
    if ($oldversion < 2023072500.01) {
        $table = new xmldb_table('local_location_institutes');
        $field = new xmldb_field('open_departmentid', XMLDB_TYPE_INTEGER, '10', null, null, null, null, 'costcenter');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        $field = new xmldb_field('open_subdepartment', XMLDB_TYPE_INTEGER, '10', null, null, null, null, 'open_departmentid');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        upgrade_plugin_savepoint(true, 2023072500.01, 'local', 'location');
    }
