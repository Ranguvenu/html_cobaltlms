...
class assignrole {
     ...
     */
    public function get_assignable_roles(\context $context,
        $rolenamedisplay = ROLENAME_ALIAS, $withusercounts = false, $user = null) {
        global $USER, $DB;
        ...
        if ($withusercounts) {
            $extrafields = ", (SELECT count(u.id)
	                            FROM {role_assignments} cra
	                            JOIN {user} u ON cra.userid = u.id
            ...
            if (!is_siteadmin() && has_capability('local/costcenter:manage_ownorganization', $systemcontext)) {
                ...
                $extrafields .= " AND u.open_costcenterid = :open_costcenterid AND u.open_departmentid = :open_departmentid";
            }
        ...
        if (is_siteadmin($userid)  || has_capability('local/costcenter:assign_multiple_departments_manage', $context, $userid)) {
            ...
            $assignrestriction = "";
                ...
                $assignrestriction = "JOIN (SELECT DISTINCT raa.allowassign
	                            AS id
	                            FROM {role_allow_assign} raa
	                            JOIN {role_assignments} ra
	                            ON ra.roleid = raa.roleid
        ...
        $sql = "SELECT r.id, r.name, r.shortname,
	            rn.name AS coursealias $extrafields
	            FROM {role} r $assignrestriction
	            JOIN {role_context_levels} rcl
	            ON (rcl.contextlevel = :contextlevel
	            AND r.id = rcl.roleid)
	            LEFT JOIN {role_names} rn
	            ON (rn.contextid = :coursecontext
	            AND rn.roleid = r.id)
