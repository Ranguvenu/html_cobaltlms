...
$PAGE->set_url('/local/courses/courseenrol.php', array('id' => $course_id, 'enrolid' => $instance->id,'programid'=>$programid,'roleid'=>$roleid,'costcenterid'=>$costcenteridstd));
} 
if($roleid == 5){
$PAGE->set_url('/local/courses/courseenrol.php', array('id' => $course_id, 'enrolid' => $instance->id,'programid'=>$programid,'roleid'=>$roleid,'costcenterid'=>$costcenteridstd, 'batchid'=>$batchid));
}
...
if($programid>0){
  ...
  $PAGE->navbar->add(get_string('programname', 'local_program', $programname), new moodle_url('/local/program/view.php?bcid='.$programid));
}
...
// $filterlist = get_filterslist();
    if (is_siteadmin() || has_capability('local/costcenter:manage_multiorganizations', $context)) {
      $filterlist = array('dependent_fields','empemail');
      ...
      $filterlist = array('dependent_fields','empemail');
    } else if (!is_siteadmin() && has_capability('local/costcenter:manage_ownsubdepartments', $context )) {
      $filterlist = array('empemail');
    }
  }
  // roleid = 5 is for student
  if($roleid == 5){
    if (is_siteadmin() || has_capability('local/costcenter:manage_multiorganizations', $context)) {
      ...
      $filterlist = array('subdepartment','email');
    }
  }
...
if ($datasubmitted->$thirdlevel == '_qf__force_multiselect_submission') {
  ...
  $mform = new filters_form($PAGE->url, array('filterlist' => $filterlist, 'enrolid' => $enrolid, 'courseid' => $course_id, 'filterparams' => $filterparams, 'action' => 'user_enrolment'), 'post', '', null, true, (array)data_submitted());
  if ($mform->is_cancelled()) {
    ...
    $subdepartment = !empty($filterdata->$thirdlevel) ? implode(',', (array)$filterdata->$thirdlevel) : null;
  ...
  $options = array('context' => $context->id, 'courseid' => $course_id, 'organization' => $organization, 'department' => $department, 'subdepartment' => $subdepartment, 'email' => $email, 'idnumber' => $idnumber, 'uname' => $uname, 'groups' => $groups, 'hrmsrole' => $hrmsrole, 'roleid' => $roleid, 'batchid' => $batchid, 'location' => $location);
	$dataobj = $course_id;
	$fromuserid = $USER->id;
  if ( $add && confirm_sesskey()) {
    ...
    if (!empty($userstoassign)) {
      ...
      if($roleid == 3){
        ...
        $continue .= '<a href='.$CFG->wwwroot.'/local/courses/courseenrol.php?id='.$instance->courseid.'&enrolid='.$instance->id.'&roleid='.$roleid.'&programid='.$userprogramid.'&costcenterid='.$usercostcenterid.' class="singlebutton"><button class="btn">'.get_string('click_continue', 'local_program').'</button></a>';
      }
      if($roleid == 5){
        $continue = '<div class="col-lg-12 col-md-12 pull-center text-center mt-15">';
        $continue .= '<a href='.$CFG->wwwroot.'/local/courses/courseenrol.php?id='.$instance->courseid.'&enrolid='.$instance->id.'&roleid='.$roleid.'&programid='.$userprogramid.'&costcenterid='.$usercostcenterid.'&batchid='.$userbatchid.' class="singlebutton"><button class="btn">'.get_string('click_continue', 'local_program').'</button></a>';
      }
...
echo $OUTPUT->footer();
  ...
  if ($remove&& confirm_sesskey()) {
    ...
    if (!empty($userstounassign)) {
          ...
          $type = $DB->get_field('user', 'open_type', array('id' => $removeuser));
					$progressbar->progress($progress);
					$progress++;
