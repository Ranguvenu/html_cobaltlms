...
if (empty($importid)) {
    ...
    if ($form1data = $mform1->get_data()) {
        ...
        echo $OUTPUT->header();
        echo html_writer::link(new moodle_url('/local/courses/courses.php'), 'Back', array('id'=>'download_courses','class' => 'btn btn-primary '));
        echo html_writer::link(new moodle_url('/local/courses/upload/sample.php?format=csv'), 'Sample', 
                array('id'=>'download_courses','class' => 'btn btn-primary '));
        echo html_writer::link(new moodle_url('/local/courses/upload/coursehelp.php'), 'Help manual', 
