...
if($featured == 1 && $featured_course){
	...
	$html = html_writer::tag('a',
			...
			echo $html;
} else if ($featured == 0 && $featured_course){
	$custom = new local_courses\action\update();
	$update = $custom->featured_course($featured_course,$featured);
  	$requestid->id = $featured_course;
	$featured_value = 1;
	$html = html_writer::tag('a',
				...
				'title' => get_string('featuredcourses', 'local_courses'), 'alt' => get_string('featuredcourses', 'local_courses'),
				'onClick' => 'featuredcourses(' . $requestid->id . ','.$featured_value.')', 
				'class'=>'myFunction', 'style'=>'width:18px;height:18px;padding: 0px;')), 
				array('href' => 'javascript:void(0)', 'featured_id' => $requestid->id, 'featured' =>$featured_value, 'sesskey' => sesskey() ));
	echo $html;
}
