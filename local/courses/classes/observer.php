...
class local_courses_observer extends \core\event\course_viewed {
    ...
    public static function course_viewed(\core\event\course_viewed $event) {
        ...
        if (!(is_siteadmin() OR has_capability('local/costcenter:manage_multiorganizations', $systemcontext))) {
            ...
            if (!$exist) {
                $user_costcenter =$DB->get_record('user',array('id'=>$USER->id),  $fields='id,open_costcenterid,open_departmentid,open_subdepartment');
                $course_costcenter =$DB->get_record('course',array('id'=>$COURSE->id),  $fields='id,open_costcenterid,open_departmentid,open_identifiedas');
                if (has_capability('local/costcenter:manage_ownorganization',$systemcontext)) {
                    ...
                    if (($user_costcenter->open_costcenterid != $course_costcenter->open_costcenterid)) {
                        $message = get_string('notyourorgcourse_msg','local_courses', $labelstring);

                        if ($course_costcenter->open_identifiedas==6){
                            redirect($CFG->wwwroot.'/local/opencourses/opencourses.php', $message, null, NOTIFY_ERROR);
                            die;
                        } else {
                ...
                } elseif (!has_capability('local/costcenter:manage_ownorganization', $systemcontext) && has_capability('local/costcenter:manage_owndepartments',$systemcontext)) {

                    if (($user_costcenter->open_costcenterid != $course_costcenter->open_costcenterid) ||
                        ($user_costcenter->open_departmentid != $course_costcenter->open_departmentid && $course_costcenter->open_departmentid != 0)) {
                        $message = get_string('notyourdeptcourse_msg','local_courses', $labelstring);
                        if ($course_costcenter->open_identifiedas==6){
                            redirect($CFG->wwwroot.'/local/opencourses/opencourses.php', $message, null, NOTIFY_ERROR);
                            die;
                        } else {
                ...
                } else if (!has_capability('local/costcenter:manage_ownorganization', $systemcontext) && !has_capability('local/costcenter:manage_owndepartments',$systemcontext) && has_capability('local/costcenter:manage_ownsubdepartments',$systemcontext)) {

                    if (($user_costcenter->open_costcenterid != $course_costcenter->open_costcenterid) || 
                        ($user_costcenter->open_departmentid != $course_costcenter->open_departmentid && $course_costcenter->open_departmentid != 0)
                        ||($user_costcenter->open_subdepartment != $course_costcenter->open_subdepartment && $course_costcenter->open_subdepartment != 0)) {
                        $message = get_string('notyoursubdeptcourse_msg','local_courses', $labelstring);
                        redirect($CFG->wwwroot/*.'/local/courses/courses.php'*/, $message, null, NOTIFY_ERROR);
                        die;
                    }
                }
                elseif (!has_capability('local/costcenter:manage_ownorganization', $systemcontext) && !has_capability('local/costcenter:manage_owndepartments',$systemcontext) && has_capability('local/costcenter:manage_ownsubdepartments',$systemcontext)) {

                    if (($user_costcenter->open_costcenterid != $course_costcenter->open_costcenterid) || 
                        ($user_costcenter->open_departmentid != $course_costcenter->open_departmentid)
                        ||($user_costcenter->open_subdepartment != $course_costcenter->open_subdepartment)) {
                        $message = get_string('notyoursubdeptcourse_msg','local_courses', $labelstring);
                        redirect($CFG->wwwroot/*.'/local/courses/courses.php'*/, $message, null, NOTIFY_ERROR);
                        die;
                    }
