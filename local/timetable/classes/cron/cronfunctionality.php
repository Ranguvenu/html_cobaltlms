...
class cronfunctionality{
    ...
    * for inserting record in local_userssyncdata.
    ...
    public function  main_hrms_frontendform_method($cir,$filecolumns, $formdata){
        ...
        while($line=$cir->next()){
            ...
            $user->username=$user->teacher_email;
            $user->coursename=$user->course_name; 
            $user->courseshortname=$user->coursecode; 
        ...
        if($this->data){
            ...
            echo'<div class="critera_error">'.\core\notification::add(get_string('provideorganization','local_employees'), \core\output\notification::NOTIFY_ERROR).'</div>';
    ...
    private function required_fields_validations($excel,$option=0){
        ...
        //------employee code validation------- 
         if ( array_key_exists('employee_id', (array)$excel) ) {
            ...
            $costcenter_shortname = core_text::strtolower($excel->organization);
            $costcenterinfo = $DB->get_record_sql("SELECT * FROM {local_costcenter} WHERE lower(shortname) = '$costcenter_shortname'");
            if ( array_key_exists('organization', (array)$excel) ) {
                ...
                echo '<div class="local_employees_sync_error">'.get_string('error_organizationcolumn_heading', 'local_users').'</div>';
                $this->errormessage = get_string('error_organizationcolumn_heading', 'local_users');
                $this->errorcount++;
            }

            //check department
            $exceldepartment =core_text::strtolower($excel->department);
            ...
            //check subdepartment
            $excelsubdepartment =core_text::strtolower($excel->subdepartment);
            if($excelsubdepartment != 'all'){
                $department_shortname = core_text::strtolower($excel->subdepartment);
                $departmentinfo = $DB->get_record_sql("SELECT * FROM {local_costcenter} WHERE lower(shortname) = '$department_shortname'");
               }

            if ( array_key_exists('subdepartment', (array)$excel) ) {
            ...
            $vardepart = $DB->get_field('local_costcenter','id',array('shortname' => $exceldepartment));
            $varsubdepart = $DB->get_field('local_costcenter','parentid',array('shortname' => $excelsubdepartment));
            $varsubdepart_exists = $DB->record_exists('local_costcenter', array('shortname' => $excelsubdepartment));
            if($varorg == $varorg1) {
                if($vardepart != $varsubdepart && !empty($varsubdepart_exists)){
                    $stringhelper = new stdClass();
                            $stringhelper->linenumber = $this->excel_line_number;
                            $stringhelper->subdepartment = $excelsubdepartment;
                            $error_string = get_string('subdepartmentisnotexists', 'local_users', $excel);
                            echo "<div class='local_users_sync_error'>".$error_string.".</div>";
                            $this->errors[] = $error_string;
                            $this->mfields[] = 'subdepartment';
                            $this->errorcount++;
                }
            } if($varorg != $varorg1 && !empty($varsubdepart_exists)){
                if($vardepart != $varsubdepart && !empty($varsubdepart_exists)){
                    $stringhelper = new stdClass();
                            $stringhelper->linenumber = $this->excel_line_number;
                            $stringhelper->department = $exceldepartment;
                            $error_string = get_string('departmentisnotexists', 'local_users', $excel);
                            echo "<div class='local_users_sync_error'>".$error_string.".</div>";
                            $this->errors[] = $error_string;
                            $this->mfields[] = 'department';
                            $this->errorcount++;
    ...
    */
    ...
    private function preparing_user_object($excel, $formdata=null){
        ...
        $user->open_costcenterid = $DB->get_field('local_costcenter', 'id', array('shortname' => $excel->organization));
        $user->open_departmentid = $DB->get_field('local_costcenter', 'id', array('shortname' => $excel->department));
        $user->open_subdepartment = $DB->get_field('local_costcenter', 'id', array('shortname' => $excel->subdepartment));
