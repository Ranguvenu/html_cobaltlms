...
$sessioncourse = $DB->get_record('attendance', array('id' => $session->attendanceid), '*', MUST_EXIST);

$cm = get_coursemodule_from_instance('attendance', $session->attendanceid, $sessioncourse->course);
require_login($session->course, $cm);
...
$PAGE->set_url('/local/attendance/password.php');
$PAGE->set_pagelayout('popup');

$PAGE->set_context(context_system::instance());

$PAGE->set_title(get_string('showqrcode', 'attendance'));
...
if ($showqr) {
    local_attendance_renderqrcode($session);
}

if ($rotateqr) {
    // attendance_generate_passwords($session);
    local_attendance_renderqrcoderotate($session);
}
