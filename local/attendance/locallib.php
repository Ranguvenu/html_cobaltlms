 ...
 * @return array
 */
function local_attendance_get_statuses($attid, $onlyvisible=true, $statusset = -1) {
    global $DB;
 ...
 * @return string
 */
function local_attendance_get_setname($attid, $statusset, $includevalues = true) {
    $statusname = get_string('statusset', 'mod_attendance', $statusset + 1);
    if ($includevalues) {
        $statuses = local_attendance_get_statuses($attid, true, $statusset);
        $statusesout = array();
 ...
 * @return array
 */
function local_attendance_get_user_sessions_log_full($userid, $pageparams) {
    global $DB;
 ...
 * @return array
 */
function local_attendance_get_user_courses_attendances($userid) {
    global $DB;
 ...
 */
function local_attendance_calc_fraction($part, $total) {
    if ($total == 0) {
        return 0;
    } else {
        return $part / $total;
 ...
 */
function local_attendance_has_logs_for_status($statusid) {
    global $DB;
 ...
 */
function local_attendance_form_sessiondate_selector (MoodleQuickForm $mform) {
 ...
 * @return int
 */
function local_attendance_get_max_statusset($attendanceid) {
    global $DB;
 ...
 * @return array
 */
function local_attendance_get_statusset_maxpoints($statuses) {
 ...
 * @param array $userids
 */
function local_attendance_update_users_grade($attendance, $userids=array()) {
    global $DB;
 ...
 */
function local_attendance_update_users_grades_by_id($attendanceid, $grade, $userids) {
    global $DB;
    ...
    $summary = new local_attendance_summary($attendanceid, $userids);
 ...
 * @return bool
 */
function local_attendance_add_status($status) {
    global $DB;
 ...
 * @param stdClass $cm
 */
function local_attendance_remove_status($status, $context = null, $cm = null) {
    global $DB;
    if (empty($context)) {
 ...
 * @return array
 */
function local_attendance_update_status($status, $acronym, $description, $grade, $visible,
 ...
 * @return string
 */
function local_attendance_random_string($length=6) {
 ...
 */
function local_attendance_can_student_mark($sess, $log = true) {
    ...
    if ($canmark && !empty($sess->preventsharedip)) {
        ...
        if (!empty($record)) {
            ...
            if ($log) {
                ...
                $event = \local_attendance\event\session_ip_shared::create(array(
                    'objectid' => 0,
                    'context' => \context_module::instance($cm->id),
                    'other' => array(
 ...
 *
 */
function local_attendance_exporttotableed($data, $filename, $format) {
    global $CFG;
 ...
 *
 */
function local_attendance_exporttocsv($data, $filename) {
    $filename .= ".txt";
 ...
 * @return array.
 */
function local_attendance_construct_sessions_data_for_add($formdata, mod_attendance_structure $att) {
    global $CFG;
...
/**
 * Helper function for local_attendance_construct_sessions_data_for_add().
 *
 * @param stdClass $formdata
 * @param stdClass $sessions
 * @param stdClass $sess
 */
function local_attendance_fill_groupid($formdata, &$sessions, $sess) {
    if ($formdata->sessiontype == local_attendance_structure::SESSION_COMMON) {
        $sess = clone $sess;
        $sess->groupid = 0;
        $sessions[] = $sess;
    } else {
 ...
 * @return stdClass
 */
function local_attendance_course_users_points($courseids = array(), $orderby = '') {
    global $DB;
 ...
 * @return stdClass
 */
function local_attendance_get_users_to_notify($courseids = array(), $orderby = '', $allfornotify = false) {
    global $DB, $CFG;
 ...
 */
function local_attendance_template_variables($record) {
    $templatevars = array(
 ...
 * @return bool/int
 */
function local_attendance_session_get_highest_status(local_attendance_structure $att, $attforsession, $scantime = null) {
 ...
 * @return array
 */
function local_attendance_get_automarkoptions() {
 ...
 */
function local_attendance_get_coursemodulenames($id) {
 ...
 * @return array
 */
function local_attendance_get_sharedipoptions() {
    $options = array();
 ...
 */
function local_attendance_strftimehm($time) {
 ...
 */
function local_attendance_construct_session_time($datetime, $duration) {
    $starttime = local_attendance_strftimehm($datetime);
    $endtime = local_attendance_strftimehm($datetime + $duration);
 ...
 * @return string.
 */
function local_construct_session_full_date_time($datetime, $duration) {
    $sessinfo = userdate($datetime, get_string('strftimedmyw', 'attendance'));
    $sessinfo .= ' '.local_attendance_construct_session_time($datetime, $duration);
 ...
 */
function local_attendance_renderpassword($session) {
 ...
 * @param stdClass $session
 */
function local_attendance_renderqrcode($session) {
    global $CFG;

    if (strlen($session->studentpassword) > 0) {
        $qrcodeurl = $CFG->wwwroot . '/local/attendance/attendance.php?qrpass=' .
            $session->studentpassword . '&sessid=' . $session->id;
    } else {
        $qrcodeurl = $CFG->wwwroot . '/local/attendance/attendance.php?sessid=' . $session->id;
    }
 ...
 * @param stdClass $session
 */
function local_attendance_generate_passwords($session) {
    global $DB;
    $attconfig = get_config('attendance');
    $password = array();
 ...
 * @param stdClass $session
 */
function local_attendance_renderqrcoderotate($session) {
    // Load required js.
 ...
 * @param stdClass $session
 */
function local_attendance_return_passwords($session) {
    global $DB;
