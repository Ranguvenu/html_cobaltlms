...
$url = new moodle_url('/local/attendance/warnings.php');
...
if (empty($id)) {
    ...
    $tabmenu = local_attendance_print_settings_tabs('defaultwarnings');
    echo $tabmenu;
    ...
    $att = new local_attendance_structure($att, $cm, $course, $PAGE->context);
    ...
    $output = $PAGE->get_renderer('local_attendance');
    $tabs = new local_attendance\output\tabs($att, local_attendance\output\tabs::TAB_WARNINGS);
    echo $output->header();
    echo $output->render($tabs);

}

$mform = new local_attendance\form\addwarning($url, array('notid' => $notid, 'id' => $id));
...
if ($action == 'update' && !empty($notid)) {
    ...
    $addurl = new moodle_url('/local/attendance/warnings.php', array('action' => 'add', 'id' => $id));
