...
$pageparams = new local_attendance_view_page_params();
$systemcontext = context_system::instance();

$id                     = required_param('id', PARAM_INT);
$edit                   = optional_param('edit', -1, PARAM_BOOL);
$pageparams->studentid  = optional_param('studentid', null, PARAM_INT);
$pageparams->mode       = optional_param('mode', local_attendance_view_page_params::MODE_THIS_COURSE, PARAM_INT);
...
$pageparams->init($cm);
$att = new local_attendance_structure($attendance, $cm, $course, $context, $pageparams);
...
if (has_any_capability($capabilities, $context) &&
    $pageparams->mode == local_attendance_view_page_params::MODE_ALL_SESSIONS) {
...
$userdata = new local_attendance\output\user_data($att, $userid);
...
if (($formdata = data_submitted()) && !empty($formdata->sesskey) && confirm_sesskey() && $edit == -1) {
    ...
    $event = \local_attendance\event\session_report_updated::create(array(
    ...
    $event = \local_attendance\event\session_report_viewed::create(array(
        'relateduserid' => $relateduserid,
        'context' => $context,
        'other' => $params));
    $event->add_record_snapshot('course_modules', $cm);
...
$semid = "SELECT lp.id
            FROM {local_program_levels} lp
            JOIn {local_program_users} lpu ON lp.programid = lpu.programid
          WHERE lpu.userid = ?";
$semid = $DB->get_field_sql($semid, [$pageparams->studentid]);
if (is_siteadmin()
