...
$pageparams = new local_attendance_sessions_page_params();
...
if ($attforsession->rotateqrcode == 1) {
    ...
    $url = new moodle_url('/local/attendance/view.php', array('id' => $cm->id));
...
list($canmark, $reason) = local_attendance_can_student_mark($attforsession);
if (!$canmark) {
    redirect(new moodle_url('/local/attendance/view.php', array('id' => $cm->id)), get_string($reason, 'attendance'));
    exit;
}
...
$att = new local_attendance_structure($attendance, $cm, $course, $PAGE->context, $pageparams);
...
if ($attforsession->autoassignstatus && (empty($attforsession->studentpassword)) || $qrpassflag) {
    $statusid = local_attendance_session_get_highest_status($att, $attforsession);
    $url = new moodle_url('/local/attendance/view.php', array('id' => $cm->id));
    if (empty($statusid)) {
...
if (!empty($qrpass) && !empty($attforsession->autoassignstatus)) {
    ...
    if (!empty($attforsession->studentpassword) &&
        ...
        $url = new moodle_url('/local/attendance/attendance.php', array('sessid' => $id, 'sesskey' => sesskey()));
    ...
    $fromform->status = local_attendance_session_get_highest_status($att, $attforsession);
...
if ($attforsession->rotateqrcode == 1) {
    echo "a";
    $mform = new local_attendance\form\studentattendance(null,
