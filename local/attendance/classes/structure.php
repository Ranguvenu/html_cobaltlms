...
/**
 * Class definition for local_attendance_structure
 *
...
class local_attendance_structure {
    /** Common sessions */
    ...
    public function __construct(stdClass $dbrecord, stdClass $cm, stdClass $course, stdClass $context=null, $pageparams=null) {
        ...
        foreach ($dbrecord as $field => $value) {
            if (property_exists('local_attendance_structure', $field)) {
                $this->{$field} = $value;
            } else {
    ...
    public function get_filtered_sessions() : array {
        ...
        if ($this->pageparams->get_current_sesstype() > local_attendance_page_with_filter_controls::SESSTYPE_ALL) {
            $where .= " AND (groupid = :cgroup OR groupid = 0)";
        }
        $params = array(
            'aid'       => $this->id,
            // 'csdate'    => $this->course->startdate,
            'csdate'    => $this->pageparams->semesterstartdate,
        ...
        $statussetmaxpoints = local_attendance_get_statusset_maxpoints($this->get_statuses(true, true));
    ...
    public function url_manage($params=array()) : moodle_url {
        global $DB;
        $cid = $DB->get_field('course_modules', 'course', ['id' => $this->cm->id]);
        $semid = $DB->get_field('local_program_level_courses', 'levelid', ['courseid' => $cid]);
        $params = array_merge(array('tlid' => $semid), $params);
        return new moodle_url('/local/timetable/individual_session.php', $params);
    }
    ...
    public function url_managetemp($params=array()) : moodle_url {
        ...
        return new moodle_url('/local/attendance/tempusers.php', $params);
    }
    ...
    public function url_tempdelete($params=array()) : moodle_url {
        ...
        return new moodle_url('/local/attendance/tempedit.php', $params);
    }
    ...
    public function url_tempmerge($params=array()) : moodle_url {
        $params = array_merge(array('id' => $this->cm->id), $params);
        return new moodle_url('/local/attendance/tempmerge.php', $params);
    }
    ...
    public function url_sessions($params=array()) : moodle_url {
        $params = array_merge(array('id' => $this->cm->id), $params);
        return new moodle_url('/local/attendance/sessions.php', $params);
    }
    ...
    public function url_report($params=array()) : moodle_url {
        $params = array_merge(array('id' => $this->cm->id), $params);
        return new moodle_url('/local/attendance/report.php', $params);
    }
    ...
    public function url_absentee($params=array()) : moodle_url {
        $params = array_merge(array('id' => $this->cm->id), $params);
        return new moodle_url('/local/attendance/absentee.php', $params);
    }
    ...
    public function url_export() : moodle_url {
        ...
        return new moodle_url('/local/attendance/export.php', $params);
    }
    ...
    public function url_preferences($params=array()) : moodle_url {
        ...
        $params = array_merge(array('id' => $this->cm->id), $params);
        return new moodle_url('/local/attendance/preferences.php', $params);
    }
    ...
    public function url_warnings($params=array()) : moodle_url {
        ...
        $params = array_merge(array('id' => $this->cm->id), $params);
        return new moodle_url('/local/attendance/warnings.php', $params);
    }
    ...
    public function url_take($params=array()) : moodle_url {
        $params = array_merge(array('id' => $this->cm->id), $params);
        return new moodle_url('/local/attendance/take.php', $params);
    }
     ...
     * @return moodle_url of attendances.php for attendance instance
     */
    public function url_att_take($params=array()) : moodle_url {
        $params = array_merge(array('id' => $this->cm->id), $params);
        return new moodle_url('/local/attendance/take.php', $params);
    }
    ...
    public function url_view($params=array()) : moodle_url {
        $params = array_merge(array('id' => $this->cm->id), $params);
        return new moodle_url('/local/attendance/view.php', $params);
    }
    ...
    public function add_session($sess) : int {
        ...
        $sess->sessionname = $sess->sessionname;
        ...
        $sess->caleventid = 0;
        local_attendance_create_calendar_event($sess);

        $infoarray = array();
        $infoarray[] = local_construct_session_full_date_time($sess->sessdate, $sess->duration);

        // Trigger a session added event.
        $event = \local_attendance\event\session_added::create(array(
            'objectid' => $this->id,
    ...
    public function update_session_from_form_data($formdata, $sessionid) {
        ...
        $sess->sessedate = $formdata->sessiondate + $sesendtime;
        $sess->duration = $sesendtime - $sesstarttime;

        /*$sessiondate = $DB->get_record('attendance_sessions', array('id' => $sessionid));
        $sess->sessdate = $sessiondate->sessdate;
        $sess->duration = $sessiondate->duration;*/
        ...
        if (empty($sess->caleventid)) {
            ...
            local_attendance_update_calendar_event($sess);
        }

        $info = local_construct_session_full_date_time($sess->sessdate, $sess->duration);
        $event = \local_attendance\event\session_updated::create(array(
            'objectid' => $this->id,
            'context' => $this->context,
            'other' => array('info' => $info, 'sessionid' => $sessionid,
                'action' => local_attendance_sessions_page_params::ACTION_UPDATE)));
        $event->add_record_snapshot('course_modules', $this->cm);
        $event->add_record_snapshot('attendance_sessions', $sess);
        $event->trigger();
    }
    ...
    public function take_from_student($mformdata) : bool {
        ...
        // Log the change.
        $event = \local_attendance\event\attendance_taken_by_student::create(array(
            'objectid' => $this->id,
            'context' => $this->context,
            'other' => $params));
        $event->add_record_snapshot('course_modules', $this->cm);
    ...
    public function save_log($sesslog) {
        ...
        $event = \local_attendance\event\attendance_taken::create(array(
            'objectid' => $this->id,
            'context' => $this->context,
            'other' => $params));
        $event->add_record_snapshot('course_modules', $this->cm);
     ...
     * @return array
     */
    public function get_users($groupid = 0, $page = 1, $batchgroup = false) : array {
        global $DB;

        /*Dipanshu Kasera code start: Added rollno  field to show in attendance grid view  */
        
        $fields = array('username' , 'idnumber' , 'institution' , 'department', 'city', 'country', 'rollno');
        ...
        if ($this->pageparams->sort == ATT_SORT_FIRSTNAME) {
            ...
            /*Dipanshu Kasera code starts: To show the ascending order users based on rollno */

            // list($orderby, $sortparams) = users_order_by_sql('u');
            $orderby = 'u.rollno, u.id';
            
            /*Dipanshu Kasera code endsts: To show the ascending order users based on rollno */
        }
        ...
        if ($page) {
            ...
            if (!empty($this->cm->groupingid)) {
                ...
                if ($batchgroup == 0) {
                    ...
                    $startusers = ($page - 1) * $usersperpage;
                    $users = get_group_enrolled_users($this->context, 'mod/attendance:canbelisted', $batchgroup, $userfields,
                        $orderby, $startusers, $usersperpage);
                }
            }
        } else {
        ...
        /*if ($batchgroup > 0) {
            $usersperpage = $this->pageparams->perpage;
            $startusers = ($page - 1) * $usersperpage;
            $bgdata = $DB->get_records('cohort_members', ['cohortid' => $batchgroup]);
            $udata = array();
            foreach ($bgdata as $key => $bguser) {
                $param = array('id' => $bguser->userid);
                $usql = "SELECT $userfields
                           FROM {user} u
                          WHERE u.id = :id";
                $udata[$bguser->userid] = $DB->get_record_sql($usql, $param, $startusers, $usersperpage);
            }
            $users = array_intersect_key($users, $udata);
        }*/
    ...
    public function get_statuses($onlyvisible = true, $allsets = false) : array {
        if (!isset($this->statuses)) {
            ...
            $this->statuses = local_attendance_get_statuses($this->id, $onlyvisible, $statusset);
            $this->allstatuses = local_attendance_get_statuses($this->id, $onlyvisible);
        }
    ...
    public function update_users_grade($userids) {
        local_attendance_update_users_grade($this, $userids);
    }
    ...
    public function get_user_filtered_sessions_log_extended($userid) : array {
        ...
        foreach ($sessions as $sess) {
            ...
            if ($sess->statusid) {
                $attstatus = $DB->get_record('attendance_statuses', array("attendanceid" => $this->id, "id" => $sess->statusid, "visible" => 1, "deleted" => 0));
                $sess->attstatus = $attstatus->description;
                $sess->grade = $attstatus->grade;
                $sess->acronym = $attstatus->acronym;
            }
        }
    ...
    public function delete_sessions($sessionsids) {
        ...
        $event = \local_attendance\event\session_deleted::create(array(
            'objectid' => $this->id,
            'context' => $this->context,
    ...
    public function update_sessions_duration($sessionsids, $duration) {
        ...
        foreach ($sessions as $sess) {
            ...
            $event = \local_attendance\event\session_duration_updated::create(array(
                'objectid' => $this->id,
                'context' => $this->context,
                'other' => array('info' => implode(', ', $sessionsids))));
            $event->add_record_snapshot('course_modules', $this->cm);
