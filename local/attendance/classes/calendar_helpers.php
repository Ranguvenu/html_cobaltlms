 ...
 */
function local_attendance_create_calendar_event(&$session) {
    global $DB;
 ...
 */
function local_attendance_create_calendar_events($sessionsids) {
    global $DB;
    ...
    foreach ($sessions as $session) {
        local_attendance_create_calendar_event($session);
        if ($session->caleventid) {
 ...
 */
function local_attendance_update_calendar_event($session) {
    global $DB;
    ...
    if ($session->caleventid == 0) {
        return local_attendance_create_calendar_event($session);
    }
 ...
 * @return bool result of updating
 */
function local_attendance_delete_calendar_events($sessionsids) {
    global $DB;
    $caleventsids = local_attendance_existing_calendar_events_ids($sessionsids);
    if ($caleventsids) {
 ...
 */
function local_attendance_existing_calendar_events_ids($sessionsids) {
    global $DB;
