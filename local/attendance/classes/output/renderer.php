...
use local_attendance_view_page_params;
use local_attendance_take_page_params;
use local_attendance_page_with_filter_controls;
use local_attendance_preferences_page_params;
use local_attendance_structure;
use local_attendance_sessions_page_params;
use local_attendance_user_sessions_cells_html_generator;
use html_table;
use html_table_row;
use html_table_cell;
use html_writer;
...
class renderer extends plugin_renderer_base {
    ...
    protected function render_filter_controls(filter_controls $fcontrols) {
        ...
        if (property_exists($fcontrols->pageparams, 'mode') &&
            $fcontrols->pageparams->mode === local_attendance_view_page_params::MODE_ALL_SESSIONS) {
            $classes .= ' float-right';
    ...
    protected function render_sess_group_selector(filter_controls $fcontrols) {
        switch ($fcontrols->pageparams->selectortype) {
            case local_attendance_page_with_filter_controls::SELECTOR_SESS_TYPE:
                ...
                break;
            case local_attendance_page_with_filter_controls::SELECTOR_GROUP:
    ...
    protected function render_grouping_controls(filter_controls $fcontrols) {
        if ($fcontrols->pageparams->mode === local_attendance_view_page_params::MODE_ALL_SESSIONS) {
            $groupoptions = array(
    ...
    protected function render_sess_manage_table(manage_data $sessdata) {
        ...
        foreach ($sessdata->sessions as $key => $sess) {
            ...
            $table->data[$sess->id][] = $get_sessioname;
            $table->data[$sess->id][] = $dta['date'];
            $table->data[$sess->id][] = $dta['time'];
            $table->data[$sess->id][] = $fullname;
            if ($sess->groupid) {
            ...
            /*DM-120 Naveen Yada made few changes */
            $userratings = $DB->get_record_sql("SELECT avg(rating) as avgrating FROM {ratings} WHERE sessionid = $sess->id");
            ...
            // $percentrating = ($avgrating * 100)/5;
            $table->data[$sess->id][] = $overallrating;
            // $table->data[$sess->id][] = $avgrating.'/5';
            /* end of the changes*/
    ...
    private function construct_date_time_actions(manage_data $sessdata, $sess) {
        ...
        if ($sess->lasttaken > 0) {
            ...
            if (has_capability('mod/attendance:takeattendances', $sessdata->att->context)) {
                ...
                /*Dipanshu Kasera code starts: To display grid view as default added viewmode*/

                $sess->viewmode = 2;
                $url = $sessdata->url_att_take($sess->id, $sess->groupid, $sess->viewmode);
        ...
        if (has_capability('mod/attendance:manageattendances', $sessdata->att->context)) {
            $url = $sessdata->url_sessions($sess->id, local_attendance_sessions_page_params::ACTION_UPDATE);
            // lasttakenby
            // lasttaken
            $title = get_string('editsession', 'attendance');
            // Sandeep - ODL4-162 Session's count were not displaying - Starts//
            // Setting gear will be accessed only, when the attendance is not taken 
            if($sess->lasttakenby <= 0 && $sess->lasttaken <= 0){
                $actions .= $this->output->action_icon($url, new pix_icon('t/edit', $title));
            // Sandeep - ODL4-162 Session's count were not displaying - Ends //

            }
                $url = $sessdata->url_sessions($sess->id, local_attendance_sessions_page_params::ACTION_DELETE);
    ...
    protected function render_sess_manage_control(manage_data $sessdata) {
        ...
        if (has_capability('mod/attendance:manageattendances', $sessdata->att->context)) {
            ...
            $options = array(local_attendance_sessions_page_params::ACTION_DELETE_SELECTED => get_string('delete'),
                local_attendance_sessions_page_params::ACTION_CHANGE_DURATION => get_string('changeduration', 'attendance'));
    ...
    protected function render_take_data(take_data $takedata) {
        ...
        if ($takedata->pageparams->viewmode == local_attendance_take_page_params::SORTED_LIST) {
            $table .= $this->render_attendance_take_list($takedata);
        } else {
            $table .= $this->render_attendance_take_grid($takedata);
        }
        $table .= html_writer::input_hidden_params(
            $takedata->url(
                array(
                    'sesskey' => sesskey(),
        ...
        $table = html_writer::tag('form', $table, array('method' => 'post', 'action' => $takedata->url_path(), 'id' => 'attendancetakeform'));
    ...
    protected function render_attendance_take_controls(take_data $takedata) {
        ...
        $url = new moodle_url('/local/attendance/import/marksessions.php', $urlparams);
    ...
    private function construct_take_controls(take_data $takedata) {
        ...
        $group = 0;
        if ($takedata->pageparams->grouptype != local_attendance_structure::SESSION_COMMON) {
        ...
        if (!empty($takedata->cm->groupingid)) {
            ...
            $totalusers = count_enrolled_group_users($context, 'mod/attendance:canbelisted', $batchgroup);
        }
        ...
        if (!empty($takedata->pageparams->page) && $takedata->pageparams->page && $totalusers && $usersperpage) {
            ...
            if ($takedata->pageparams->page > 1) {
                $controls .= html_writer::link($takedata->url(array('semid' => $takedata->pageparams->semid, 'batch_group' => $batchgroup, 'page' => $takedata->pageparams->page - 1)),
                                                              $this->output->larrow());
            }
            $a = new stdClass();
        ...
        $options = array(
            local_attendance_take_page_params::SORTED_LIST   => get_string('sortedlist', 'attendance'),
            local_attendance_take_page_params::SORTED_GRID   => get_string('sortedgrid', 'attendance'));
        $select = new single_select($takedata->url(array('semid' => $takedata->pageparams->semid, 'batch_group' => $batchgroup)), 'viewmode', $options, $takedata->pageparams->viewmode, null);
        ...
        if ($takedata->pageparams->viewmode == local_attendance_take_page_params::SORTED_GRID) {
            ...
            $select = new single_select($takedata->url(array('semid' => $takedata->pageparams->semid, 'batch_group' => $batchgroup)), 'gridcols', $options, $takedata->pageparams->gridcols, null);
            $select->class = 'singleselect inline';
            $controls .= $this->output->render($select);
        }
        if (/*isset($takedata->sessions4copy) && */count($takedata->sessions4copy) > 0) {
            ...
            $select = new single_select($takedata->url(array('semid' => $takedata->pageparams->semid, 'batch_group' => $batchgroup), array('copyfrom')), 'copyfrom', $options);
    ...
    private function statusdropdown() {
        ...
        $options = array('all' => get_string('statusall', 'local_attendance'),
            'unselected' => get_string('statusunselected', 'local_attendance'));
    ...
    protected function render_attendance_take_grid(take_data $takedata) {
                 ...
                 require(['jquery'], function($) {
                     $('#checkstatus".$st->id."').click(function(e) {
                         if ($('select[name=\"setallstatus-select\"] option:selected').val() == 'unselected') {
                             ...
                             $('#attendancetakeform').find('.st".$st->id."').prop('checked', true);
                             M.util.set_user_preference('mod_attendance_statusdropdown','all');
                             if ($('#checkstatus".$st->id."').html() === 'P') {
                                $('.generaltable.takegrid td.userwithoutenrol').addClass('present');
                                $('.generaltable.takegrid td.userwithoutenrol').removeClass('absent');
                                $('.generaltable.takegrid td.userwithoutenrol').removeClass('late');
                             }
                             if ($('#checkstatus".$st->id."').html() === 'A') {
                                $('.generaltable.takegrid td.userwithoutenrol').addClass('absent');
                                $('.generaltable.takegrid td.userwithoutenrol').removeClass('present');
                                $('.generaltable.takegrid td.userwithoutenrol').removeClass('late');
                             }
                             if ($('#checkstatus".$st->id."').html() === 'L') {
                                $('.generaltable.takegrid td.userwithoutenrol').addClass('late');
                                $('.generaltable.takegrid td.userwithoutenrol').removeClass('present');
                                $('.generaltable.takegrid td.userwithoutenrol').removeClass('absent');
                             }
                         }
        ...
        $row = new html_table_row();
        foreach ($takedata->users as $user) {
            $celltext = html_writer::start_div('attendance_cards');
            $celltext .= $this->user_picture($user, array('size' => 60, 'class' => 'userpicture', 'link' => false)); // Show different picture if it is a temporary user.
            ...
            /* Dipanshu Kasera code starts: Added rollno*/

            $celltext .= html_writer::tag('span', $user->rollno, array('class' => 'rollno'));
            $celltext .= html_writer::empty_tag('br');

            /* Dipanshu Kasera code ends: Added rollno*/

            $celltext .= html_writer::tag('span', $fullname, array('class' => 'fullname'));
            $celltext .= html_writer::empty_tag('br');
            $celltext .= html_writer::empty_tag('input', array('type'=>'hidden', 'name'=>'jsonstatus'.$user->id, 'class'=>'jsonstatus'.$user->id, 'value'=>json_encode($takedata->statuses), 'style'=>'display: none;'));
            $celltext .= html_writer::empty_tag('input', array('type'=>'hidden', 'class'=>'user'.$user->id, 'name'=>'user' . $user->id, 'value'=>''));
            $celltext .= is_array($ucdata['text']) ? implode('', $ucdata['text']) : $ucdata['text'];
            if (array_key_exists('warning', $ucdata)) {
                $celltext .= html_writer::empty_tag('br');
                $celltext .= $ucdata['warning'];
            }
            $celltext .= html_writer::end_div();
    ...
    private function construct_take_user_controls(take_data $takedata, $user) {
        global $DB;
        $this->page->requires->js_call_amd('local_attendance/attendanceview');
        $celldata = array();
        if ($user->enrolmentend and $user->enrolmentend < $takedata->sessioninfo->sessdate) {
            ...
            $celldata['class'] = 'userwithoutenrol userwithoutenrol'.$user->id;
            ...
            $exist = array();
            foreach ($takedata->statuses as $st) {
                $exist[$st->id] = $st->acronym;
                $params = array(
                        'type'  => 'radio',
                        'name'  => 'user'.$user->id,
                        // 'class' => 'st'.$st->id,
                        'value' => $st->id,
                        'onclick' => 'enablecomment('.$user->id.')');

                if ($st->acronym == 'P') {
                    $params['id'] = 'local_attendance_green';
                    $params['class'] = 'local_attendance_green'.$user->id.' st'.$st->id;
                    $params['checked'] = 'checked';
                } else if ($st->acronym == 'A') {
                    $params['id'] = 'local_attendance_red';
                    $params['class'] = 'local_attendance_red'.$user->id.' st'.$st->id;

                } /*else if ($st->acronym == 'E') {
                    $params['id'] = 'local_attendance_brown';
                    $params['class'] = 'local_attendance_brown'.$user->id;

                }*/ else {
                    $params['id'] = 'local_attendance_yellow';
                    $params['class'] = 'local_attendance_yellow'.$user->id.' st'.$st->id;
                }
            ...
            $params = array(
                    ...
                    'maxlength' => 255,
                    'id' => 'representation_comment'.$user->id,
                    /*'style' => 'display: none;'*/);
            if (array_key_exists($user->id, $takedata->sessionlog)) {
                $params['value'] = $takedata->sessionlog[$user->id]->remarks;
                 if ($exist[$takedata->sessionlog[$user->userid]->statusid] == 'L') {
                    $params['style'] = 'display: block;';
                }
            }

            $colors = array('P' => 'present', 'A' => 'absent', 'L' => 'late', /*'E' => 'brown'*/);
            $class = 'present';
            $status = 'P';
            $absent = 'A';
            $late = 'L';
            ...
            if (array_key_exists($user->id, $takedata->sessionlog)){
                $class = $colors[$exist[$takedata->sessionlog[$user->id]->statusid]];
                if ($exist[$takedata->sessionlog[$user->id]->statusid] == 'P') {
                    $status = $exist[$takedata->sessionlog[$user->id]->statusid];
                }
                if ($exist[$takedata->sessionlog[$user->id]->statusid] == 'A') {
                    $absent = $exist[$takedata->sessionlog[$user->id]->statusid];
                }
                if ($exist[$takedata->sessionlog[$user->id]->statusid] == 'L') {
                    $late = $exist[$takedata->sessionlog[$user->id]->statusid];
                }
            }

            $attsql = "SELECT st.id
                        FROM {attendance_statuses} st
                        JOIN {attendance_sessions} ats ON st.attendanceid = ats.attendanceid
                       WHERE ats.id = ?";
            $attids = $DB->get_records_sql($attsql, [$takedata->pageparams->sessionid]);
            $ids = array_keys($attids);

            $celldata['text'][] = html_writer::start_div('status');

            $celldata['text'][] = html_writer::tag('div', $status, array('class' => 'attendancestatus present attstatus'.$ids[0].' u'.$user->id));
            $celldata['text'][] = html_writer::tag('div', $absent, array('class' => 'attendancestatus absent attstatus'.$ids[1].' u'.$user->id));
            $celldata['text'][] = html_writer::tag('div', $late, array('class' => 'attendancestatus late attstatus'.$ids[2].' u'.$user->id));
            $celldata['text'][] = html_writer::empty_tag('br');
            $celldata['text'][] = html_writer::end_div();
            ...
            // if ($user->enrolmentstart > $takedata->sessioninfo->sessdate + $takedata->sessioninfo->duration) {
                ...
                $celldata['class'] = 'userwithoutenrol userwithoutenrol'.$user->id.' '.$class;
            // }
        }
    ...
    protected function render_user_data(user_data $userdata) {
        ...
        if ($USER->id == $userdata->user->id ||
            $userdata->pageparams->mode === local_attendance_view_page_params::MODE_ALL_SESSIONS) {
    ...
    protected function render_user_report_tabs(user_data $userdata) {
        ...
        $tabs[] = new tabobject(local_attendance_view_page_params::MODE_THIS_COURSE,
                        $userdata->url()->out(true, array('mode' => local_attendance_view_page_params::MODE_THIS_COURSE)),
        ...
        if ($userdata->user->type == 'standard') {
            $tabs[] = new tabobject(local_attendance_view_page_params::MODE_ALL_COURSES,
                            $userdata->url()->out(true, array('mode' => local_attendance_view_page_params::MODE_ALL_COURSES)),
                            get_string('allcourses', 'attendance'));
            $tabs[] = new tabobject(local_attendance_view_page_params::MODE_ALL_SESSIONS,
                            $userdata->url()->out(true, array('mode' => local_attendance_view_page_params::MODE_ALL_SESSIONS)),
    ...
    private function construct_user_data(user_data $userdata) {
        ...
        if ($userdata->pageparams->mode == local_attendance_view_page_params::MODE_THIS_COURSE) {
        ...
        } else if ($userdata->pageparams->mode == local_attendance_view_page_params::MODE_ALL_SESSIONS) {
            ...
            foreach ($userdata->coursesatts as $ca) {
                ...
                $attendanceurl = new moodle_url('/local/attendance/view.php', array('id' => $ca->cmid,
                                                                                      'studentid' => $userdata->user->id,
                                                                                      'view' => ATT_VIEW_ALL));
                // $row->cells[] = html_writer::link($attendanceurl, $ca->attname);
                $row->cells[] = ucfirst($ca->attname);
    ...
    private function construct_user_sessions_log(user_data $userdata) {
        ...
        $coursecontext = \context_course::instance($userdata->filtercontrols->cm->course);
        $shortform = false;
        if ($USER->id == $userdata->user->id) {
        ...
        $table->head[] = get_string('remarks', 'attendance');
        $table->head[] = get_string('instructor', 'attendance');
        $table->head[] = get_string('ratings', 'attendance');

        $table->align = array_merge($table->align, array('', 'left', 'center', 'center', 'center', 'center', 'center'));
        $table->colclasses = array_merge($table->colclasses, array('datecol', 'desccol', 'statuscol', 'pointscol', 'remarkscol' , 'instructorcol', 'ratingscol'));
        ...
        $statussetmaxpoints = local_attendance_get_statusset_maxpoints($userdata->statuses);
        ...
        foreach ($userdata->sessionslog as $sess) {
            ...
            if (!empty($sess->statusid)) {
                ...
                list($canmark, $reason) = local_attendance_can_student_mark($sess, false);
                if ($canmark) {
                    ...
                    $cell = new html_table_cell(html_writer::span('NA'));
                    }
                    $cell->colspan = 3;
                    $row->cells[] = $cell;
            ...
            /*if (has_capability('mod/attendance:takeattendances', $context)) {
                ...
                $url = new moodle_url('/local/attendance/take.php', $params);
                $icon = $this->output->pix_icon('redo', get_string('changeattendance', 'attendance'), 'attendance');
                $row->cells[] = html_writer::link($url, $icon);
            }*/
            $instructor = $DB->get_record_sql("SELECT u.id, concat(u.firstname, ' ', u.lastname) as firstname FROM {user} u JOIN {role_assignments} ra ON ra.userid = u.id WHERE ra.roleid = 3 AND ra.contextid = $coursecontext->id");
            if ($instructor->id) {
                $row->cells[] = html_writer::span($instructor->firstname);
            } else {
                $row->cells[] = html_writer::span(get_string('noteacherassign','attendance'));
            }
            $dynamicid = 'ratingstars'.$sess->id;
            $ratedby = $USER->id;
            $sessionid = $sess->id;
            if ($instructor->id) {
                $ratingsgiven = $DB->get_record_sql("SELECT id, rating, sessionid FROM {ratings} WHERE ratedby = $USER->id AND sessionid = $sess->id AND instructorid = $instructor->id");
            }else{
                $ratingsgiven = null;
            }
            $attendancestatus = $DB->get_record('attendance_log', array('studentid' => $USER->id, 'sessionid' => $sess->id));
            $statusset = explode(',', $attendancestatus->statusset);
            if ($attendancestatus->statusid == $statusset[0] && $sess->acronym == 'P' && $sess->grade == '2.00') {
                $this->page->requires->js_amd_inline("
                    require(['core/ajax', 'jquery', 'jqueryui'],
                        function(Ajax, $){
                        $(document).ready(function () {
                            setStars(".$ratingsgiven->rating.");
                           var ratedIndex;
                           var rtdby = ".$USER->id.";
                           var sessid = ".$sessionid.";
                           var instrid = ".$instructor->id.";
                        $('#".$dynamicid." .fa-star').on('click', function () {
                        ...
                        $('#".$dynamicid." .fa-star').mouseover(function () {
                            $(this).css('cursor', 'pointer');
                            resetStarColors();
                            var currentIndex = parseInt($(this).data('index'));
                            setStars(currentIndex);
                        });
                        $('#".$dynamicid." .fa-star').mouseleave(function () {
                            if (ratedIndex != -1)
                                setStars(".$ratingsgiven->rating.");
                        });
                    });
                ...
                function saveToTheDB() {
                    $.ajax({
                       url: M.cfg.wwwroot+'/mod/attendance/ajax.php',
                       method: 'POST',
                       dataType: 'json',
                       data: {
                           save: 1,
                           ...
                           instructorid: instructorid
                            ...
                            ratedIndex = r.id;
                            localStorage.setItem('ratedInd', ratedInd);
                       }
                    });
                }
                    function setStars(max) {
                        resetStarColors();
                        for (var i=0; i < max; i++)
                            $('#".$dynamicid." .fa-star:eq('+i+')').css('color', 'darkorange');
                    }

                    function resetStarColors() {
                        $('#".$dynamicid." .fa-star').css('color', 'gray');
                    }
                });");
            }
            $row->cells[] = html_writer::tag('form','
                    <i class="fa fa-star" data-index="1"></i>
                    <i class="fa fa-star" data-index="2"></i>
                    <i class="fa fa-star" data-index="3"></i>
                    <i class="fa fa-star" data-index="4"></i>
                    <i class="fa fa-star" data-index="5"></i>', array('id' => $dynamicid));
            $table->data[] = $row;
        }
    ...
    private function construct_user_allsessions_log(user_data $userdata) {
        ...
        foreach ($userdata->statuses as $attid => $attstatuses) {
            $statusmaxpoints[$attid] = local_attendance_get_statusset_maxpoints($attstatuses);
        }
        ...
        foreach ($groups as $group) {
            ...
            foreach ($group as $sess) {
                ...
                if ($groupby !== 'activity') {
                    ...
                    $row->cells[] = ucfirst($sess->attname);
                }
                ...
                if (has_capability('mod/attendance:takeattendances', $sesscontext)) {
                    $sessionurl = new moodle_url('/local/attendance/take.php', array('id' => $sess->cmid,
                                                                                   'sessionid' => $sess->id,
                                                                                   'grouptype' => $sess->groupid));
                ...
                if (!empty($USER->attendanceediting)) {
                    ...
                    if (has_capability('mod/attendance:takeattendances', $context)) {
                        ...
                        $nastyhack = new \ReflectionClass('local\attendance\output\take_data');
                        ...
                        $takedata->pageparams->viewmode = local_attendance_take_page_params::SORTED_GRID;
    ...
    private function construct_time($datetime, $duration) {
        $time = html_writer::tag('nobr', local_attendance_construct_session_time($datetime, $duration));
        ...
        if ($bulkmessagecapability) { // Require that the user can bulk message users.
            ...
            $output .= html_writer::tag('div',
                    html_writer::empty_tag('input', array('type' => 'submit',
                                                                   ...
                                                                   'class' => 'btn btn-secondary')),
                    array('class' => 'buttons'));
            $url = new moodle_url('/local/attendance/messageselect.php');
    ...
    protected function get_acronym_rows(report_data $reportdata, $startwithcontrast=false) {
        ...
        foreach ($reportdata->statuses as $sts) {
            if ($sts->setnumber != $setnumber) {
                $contrast = !$contrast;
                $setnumber = $sts->setnumber;
                $text = local_attendance_get_setname($reportdata->att->id, $setnumber, false);
    ...
    protected function get_session_rows(report_data $reportdata, $startwithcontrast=false) {
        ...
        foreach ($reportdata->users as $user) {
            $row = new html_table_row();
            if ($showsessiondetails && !empty($reportdata->sessions)) {
                $cellsgenerator = new local_attendance_user_sessions_cells_html_generator($reportdata, $user);
    ...
    protected function render_preferences_data($prefdata) {
        ...
        $cells[] = $this->construct_preferences_button(get_string('add', 'attendance'),
            local_attendance_preferences_page_params::ACTION_ADD);
        ...
        $o .= $this->construct_preferences_button(get_string('update', 'attendance'),
                                                  local_attendance_preferences_page_params::ACTION_SAVE);
    ...
    private function construct_preferences_actions_icons($st, $prefdata) {
        $params = array('sesskey' => sesskey(),
                        'statusid' => $st->id);
        if ($st->visible) {
            $params['action'] = local_attendance_preferences_page_params::ACTION_HIDE;
            $showhideicon = $this->output->action_icon(
                    $prefdata->url($params),
                    new pix_icon("t/hide", get_string('hide')));
        } else {
            $params['action'] = local_attendance_preferences_page_params::ACTION_SHOW;
            $showhideicon = $this->output->action_icon(
                    $prefdata->url($params),
                    new pix_icon("t/show", get_string('show')));
        }
        if (empty($st->haslogs)) {
            $params['action'] = local_attendance_preferences_page_params::ACTION_DELETE;
