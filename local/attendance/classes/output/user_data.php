...
class user_data implements renderable {
     ...
     */
    public function  __construct(local_attendance_structure $att, $userid, $mobile = false) {
        $this->user = $att->get_user($userid);

        $this->pageparams = $att->pageparams;

        if ($this->pageparams->mode == local_attendance_view_page_params::MODE_THIS_COURSE) {
            $this->statuses = $att->get_statuses(true, true);

            if (!$mobile) {
                $this->summary = new local_attendance_summary($att->id, array($userid), $att->pageparams->startdate,
        ...
        } else if ($this->pageparams->mode == local_attendance_view_page_params::MODE_ALL_SESSIONS) {
            $this->coursesatts = local_attendance_get_user_courses_attendances($userid);
            $this->statuses = array();
            ...
            foreach ($this->coursesatts as $atid => $ca) {
                ...
                $this->statuses[$ca->attid] = local_attendance_get_statuses($ca->attid);
                $this->summaries[$ca->attid] = new local_attendance_summary($ca->attid, array($userid));
            ...
            $this->sessionslog = local_attendance_get_user_sessions_log_full($userid, $this->pageparams);
            ...
            foreach ($this->coursesatts as $atid => $ca) {
                ...
                $this->statuses[$ca->attid] = local_attendance_get_statuses($ca->attid);
                $this->summary[$ca->attid] = new local_attendance_summary($ca->attid, array($userid));
            }
        }
        ...
        if (!empty($updateatts)) {
            ...
            foreach ($updateatts as $attid => $updateusers) {
                if ($attendancegrade[$attid] != 0) {
                    local_attendance_update_users_grades_by_id($attid, $grade, $updateusers);
                }
            }
        }
    }
