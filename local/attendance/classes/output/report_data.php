...
class report_data implements renderable {
    ...
    /** @var array|local_attendance_summary  */
    ...
    public function  __construct(local_attendance_structure $att) {
        ...
        $this->allstatuses = local_attendance_get_statuses($att->id, false);

        if ($att->pageparams->view == ATT_VIEW_SUMMARY) {
            $this->summary = new local_attendance_summary($att->id);
        } else {
            $this->summary = new local_attendance_summary($att->id, array_keys($this->users),
        ...
        foreach ($this->users as $key => $user) {
            ...
            if ($att->pageparams->view != ATT_VIEW_NOTPRESENT ||
                local_attendance_calc_fraction($usersummary->takensessionspoints, $usersummary->takensessionsmaxpoints) <
