...
function user_get_user_navigation_info($user, $page, $options = array()) {
    ...
    $returnobject->metadata = array();
       ...
       // Links: Dashboard.
        ...
        if($user->open_type == null){
        $url = new moodle_url('/user/profile.php', array(
            'id' => $user->id
        ));
        }else if($user->open_type == 1) {
            $url = new moodle_url('/local/users/profile.php', array(
                'id' => $user->id
            ));
        } else {
            $url = new moodle_url('/local/employees/profile.php', array(
                'id' => $user->id
            ));
        }
       // Links: My Profile.
        ...
        $mygrade = new stdClass();
        $mygrade->itemtype = 'link';
        $mygrade->url = new moodle_url('/grade/report/overview/index.php');
        $mygrade->title = get_string('grades');
        $mygrade->titleidentifier = 'grades,moodle';
        // $mygrade->pix = "i/user";
        $returnobject->navitems[] = $mygrade;

        $mycalendar = new stdClass();
        $mycalendar->itemtype = 'link';
        $mycalendar->url = new moodle_url('/calendar/view.php?view=month');
        $mycalendar->title = get_string('calendar', 'local_users');
        $mycalendar->titleidentifier = 'calendar,moodle';
        // $mycalendar->pix = "i/user";
        $returnobject->navitems[] = $mycalendar;


        $myfiles = new stdClass();
        $myfiles->itemtype = 'link';
        $myfiles->url = new moodle_url('/user/files.php');
        $myfiles->title = get_string('privatefiles');
        $myfiles->titleidentifier = 'private,moodle';
        // $myfiles->pix = "i/user";
        $returnobject->navitems[] = $myfiles;


        $myreportbuild = new stdClass();
        $myreportbuild->itemtype = 'link';
        $myreportbuild->url = new moodle_url('/reportbuilder/index.php');
        $myreportbuild->title = get_string('reports');
        $myreportbuild->titleidentifier = 'private,moodle';
        // $myreportbuild->pix = "i/user";
        $returnobject->navitems[] = $myreportbuild;
