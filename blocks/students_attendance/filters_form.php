...
class filters_form extends moodleform {
    ...
    function definition() {
        ...
        foreach ($filterlist as $key => $value) {
            if ($value === 'student_courses') {
                $filter = 'students_attendance';
            } else {
                $filter = $value;
            }
        ...
        if ($action === 'user_enrolment') {
            ...
            $applyclassarray = array('class' => 'form-submit',
                                'onclick' => 
                                '(function(e){ 
                                require("block_students_attendance/cardPaginate").filteringData(e,"'.$submitid.'") })(event)');
            $cancelclassarray = array('class' => 'form-submit',
                                'onclick' => 
                                '(function(e){ require("block_students_attendance/cardPaginate").resetingData(e,"'.$submitid.'") })(event)');
