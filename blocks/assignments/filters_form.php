...
class filters_form extends moodleform {
    ...
    function definition() {
        ...
        foreach ($filterlist as $key => $value) {
            if ($value === 'assignmentname' || $value === 'coursename' || $value === 'studentname') {
                $filter = 'assignments';
            } else {
                $filter = $value;
            }
        ...
        if ($action === 'user_enrolment') {
            ...
            $applyclassarray = array('class' => 'form-submit',
                                'onclick' => 
                                '(function(e){ 
                                require("block_assignments/cardPaginate").filteringData(e,"'.$submitid.'") })(event)');
            $cancelclassarray = array('class' => 'form-submit',
                                'onclick' => 
                                '(function(e){ require("block_assignments/cardPaginate").resetingData(e,"'.$submitid.'") })(event)');
