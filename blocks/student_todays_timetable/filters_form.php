...
class filters_form extends moodleform {
    ...
    function definition() {
        ...
        foreach ($filterlist as $key => $value) {
            if ($value === 'subject' || $value === 'faculty'){
                $filter = 'student_todays_timetable';
            } else {
                $filter = $value;
            }
        ...
        if ($action === 'user_enrolment') {
            ...
            $applyclassarray = array('class' => 'form-submit',
                                'onclick' => 
                                '(function(e){ 
                                require("block_student_todays_timetable/cardPaginate").filteringData(e,"'.$submitid.'") })(event)');
            $cancelclassarray = array('class' => 'form-submit',
                                'onclick' => 
                                '(function(e){ require("block_student_todays_timetable/cardPaginate").resetingData(e,"'.$submitid.'") })(event)');
