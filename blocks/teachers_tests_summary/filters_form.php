...
class filters_form extends moodleform {
    ...
    function definition() {
        ...
        foreach ($filterlist as $key => $value) {
            if ($value === 'quizname' || $value === 'course') {
                $filter = 'teachers_tests_summary';
            } else {
                $filter = $value;
            }
        ...
        if ($action === 'user_enrolment') {
            ...
            $applyclassarray = array('class' => 'form-submit',
                                'onclick' => 
                                '(function(e){ 
                                require("block_teachers_tests_summary/cardPaginate").filteringData(e,"'.$submitid.'") })(event)');
            $cancelclassarray = array('class' => 'form-submit',
                                'onclick' => 
                                '(function(e){ require("block_teachers_tests_summary/cardPaginate").resetingData(e,"'.$submitid.'") })(event)');
