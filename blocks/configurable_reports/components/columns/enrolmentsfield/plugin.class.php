...
class plugin_enrolmentsfield extends plugin_base{

    public function init() {
        $this->fullname = get_string('enrolmentsfield', 'block_configurable_reports');
        $this->type = 'undefined';
        $this->form = true;
        $this->reporttypes = array('enrolments');
    }
    ...
    public function execute($data, $row, $user, $courseid, $starttime = 0, $endtime = 0) {
        ...
        if (strpos($data->column, 'profile_') === 0) {
            ...
            $sql = "SELECT d.* from {user} d WHERE d.userid= ?";
