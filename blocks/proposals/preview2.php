...
global $DB , $USER;
require_login();
$PAGE->set_url(new moodle_url('/blocks/proposals/preview2.php'));
...
$schoolname = $DB->get_field('local_costcenter','fullname',array('id'=>$username->open_costcenterid));
        ...
        $item = $data->attachments;
            ...
            $attachment = $DB->get_field('files','filename',array('itemid'=>$item));
    ...
    $other3sql = "SELECT filename FROM {files} WHERE filesize > 0 and itemid = $data->otherquestionnairesthr" ;
        ...
        $other5sql = "SELECT filename FROM {files} WHERE filesize > 0 and itemid = $data->otherquestionnairesfiv" ;
         ...
         if (!$informed) {
            ...
            if (!$studyprocedurefile) {
              $studyprocedurefile = 'NA';
            }
            if (!$samplesizejustrificationfile) {
              $samplesizejustrificationfile = 'NA';
            }
            if (!$patient) {
              $patient = 'NA';
            }
            ...
            $pdf->Cell(20,10,'StudyProcedure ',0,0,'L');
            $pdf->Cell(60,10,':',0,0,'C');
            $pdf->Cell(1,10,$data->studyprocedure,0,0,'L');
            $pdf->Ln(9,false);

            $pdf->Cell(20,10,'Studyprocedurefile',0,0,'L');
            $pdf->Cell(60,10,':',0,0,'C');
            $pdf->Cell(1,10,$studyprocedurefile,0,0,'L');
            $pdf->Ln(9,false);
            ...
            $pdf->Cell(20,10,'Samplesizejustrificationfile',0,0,'L');
            $pdf->Cell(60,10,':',0,0,'C');
            $pdf->Cell(1,10,$samplesizejustrificationfile,0,0,'L');
            $pdf->Ln(9,false);
