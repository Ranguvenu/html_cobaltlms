...
require_login();
$url = new moodle_url('/blocks/block_open_courses/semesterprogress_table.php');
$PAGE->requires->jquery();
 $PAGE->requires->js_call_amd('block_semester_progress/table', 'CustomClassDatatable', array());
$PAGE->set_url($url);
$PAGE->set_title(get_string('pluginname', 'block_open_courses'));
$PAGE->set_heading(get_string('pluginname', 'block_open_courses'));
$PAGE->navbar->add(get_string('home', 'block_open_courses'), new moodle_url('/my'));
$PAGE->navbar->add(get_string('pluginname', 'block_open_courses'));
require_once($CFG->dirroot . '/blocks/open_courses/lib.php');
require_once($CFG->dirroot . '/blocks/open_courses/filters_form.php');
...
//$courseprogress = round($completedmodulescount/$totalmodulescount*100) > 0 ? round($completedmodulescount/$totalmodulescount*100) : 0;
...
echo $OUTPUT->header();
    ...
    if($users) {
        ...
        $renderer = $PAGE->get_renderer('block_open_courses');
    ...
    echo        '</div>
        ...
        echo $OUTPUT->render_from_template('block_open_courses/global_filter', $filterparams);
