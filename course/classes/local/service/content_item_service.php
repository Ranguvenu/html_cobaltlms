...
class content_item_service {
    ...
    public function get_content_items_for_user_in_course(\stdClass $user, \stdClass $course, array $linkparams = []): array {
        ...
        /* Dipanshu code starts: to stop another attendance activity to be added to course*/
        $attactivitycountsql = "SELECT count(cm.id)
                                FROM mdl_course_modules cm
                                JOIN mdl_modules m ON cm.module = m.id AND m.name = 'attendance'
                                WHERE cm.course = {$course->id}";
        $attactivitycount = $DB->count_records_sql($attactivitycountsql);
        if ($attactivitycount == 1) {
           unset($exported->content_items[1]);
        }
        /* Dipanshu code ends: to stop another attendance activity to be added to course*/
