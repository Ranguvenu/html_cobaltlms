...
/* Dipanshu code starts: Attendance added in menu-link in course*/
function format_social_extend_navigation_course(navigation_node $navigation) {
    global $DB, $PAGE;

    $attendancesql = "SELECT cm.id, m.name
                        FROM mdl_course_modules cm
                        JOIN mdl_modules m ON cm.module = m.id AND m.name = 'attendance'
                        WHERE cm.course = {$PAGE->course->id}";
    $attendanceid = $DB->get_record_sql($attendancesql);

    $node = navigation_node::create(
                get_string('attendance', 'format_social'),
                new moodle_url('/mod/attendance/view.php', array('id' => $attendanceid->id)),
                navigation_node::TYPE_SETTING, null, 'attendance',
                new pix_icon('i/attendance', '')
            );

    // $navigation->add_node($node);
}
/* Dipanshu code ends: Attendance added in menu-link in course*/
